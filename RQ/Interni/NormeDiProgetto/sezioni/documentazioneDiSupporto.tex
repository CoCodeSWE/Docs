...
\begin{itemize}
	\item aprire il terminale e digitare \file{git clone} \url{https://github.com/CoCodeSWE/AtAVi.git}, per copiare la repository di AtAVi in locale;
	\item \file{cd AtAVi} per spostarsi all'interno della cartella creata;
	\item \file{npm install} per installare tutte le dipendenze e le librerie necessarie;
	\item \file{npm install -g grunt} e \file{npm install -g mocha} per avere a disposizione le due librerie e i comandi associati;
	\item \file{grunt build-client} per compilare la prima volta il client;
...
dove al posto di "***" deve essere inserito il percorso dove è stata scaricata la repository di AtAVi;
	\item installare l'estensione LiveReload nel proprio \gl{browser} (ad esempio per Google Chrome dall'indirizzo \url{https://chrome.google.com/webstore/detail/livereload/jnihajbhpnppcggbcgedagnkighmdlei?hl=it}, visitato in data 2017-05-04);
	\item aprire la pagina del localhost;
	\item digitare \file{grunt watch} nel terminale per fare in modo che la pagina html si aggiorni in modo automatico ad ogni modifica dei file presenti nella cartella AtAVi;
...
\begin{itemize}
	\item dal terminale, nella cartella contenente la repository di AtAVi, lanciare il comando \file{grunt test-client};
	\item aprire il file index.htm dal percorso \file{AtAVi/dist/test/client}, che mostrerà tutti i test eseguiti con il relativo esito.
\end{itemize}
\subsubsection{Esecuzione test back-end}
Per potere eseguire e controllare l'esito dei test del back-end è necessario:
\begin{itemize}
	\item dal terminale, nella cartella contenente la repository di AtAVi, lanciare il comando \file{npm install};
	\item spostarsi nella cartella \file{AtAVi/src/test} e lanciare il comando \file{mocha test\_NomeFile.js}. In questo modo verranno visualizzati gli esiti dei test della classe \file{NomeFile.js}.
\end{itemize}