...
esterno
\paragraph{Classi}
\subsubsection{\label{Back-end}}
Package contenente tutte le componenti che costituiscono il back-end. Le componenti sono organizzate secondo il pattern a \gl{microservizi}.   \begin{figure}[h] \centering \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/diagrams/back-end/Official_Backend_0304/Back-end.png}
...
Nel caso il JWT non sia valido, l'attributo \file{LambdaResponse::statusCode} sarà pari a 403. \\;
		\end{itemize}
		\item[] \file{+ <<Create>> createAdministrationWebhookService(jwt: JSONWebTokenModule)\\: AdministrationWebhookService} \\		Costruttore che si occupa di effettuare la dependency injection di \file{JSONWebTokenModule};\\
		Parametri:
		\begin{itemize}
			\item \file{jwt: JSONWebTokenModule} \\
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassMemberListQueryParameters.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassChannelListQueryParameters.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaChannelListEvent.png}
...
\item la definizione dei dati di un task;
...
È utilizzata dalla classe \file{RulesDAO} e dalle classi che utilizzano quest'ultima;
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> Rule(targ: RuleTargetArray, name: String, ac\_mode: \\int, ac\_list: StringArray, en: boolean, task: String): Rule} \\		Metodo che permette di instanziare un oggetto \file{Rule} a partire da un nome, una lista dei targets un \file{ac\_mode}, un \file{ac\_list}, un compito da applicare e un valore booleano per abilitarla o meno;\\
		Parametri:
		\begin{itemize}
			...
			\item \file{ac\_mode: int} \\
			Parametro contenente l'ac\_mode da assegnare alla \file{Rule};
			\item \file{ac\_list: StringArray} \\
			Parametro contenente l'array di id degli amministratori abilitati/disabilitati da assegnare alla \file{Rule};
		...
		\end{itemize}
		\item[] \file{+ setAcMode(acm: int): void} \\		Metodo che permette di passare l'\file{ac\_mode} per la \file{Rule};\\
		Parametri:
		\begin{itemize}
			\item \file{acm: int} \\
			Parametro contenente l'ac\_mode da passare;
		\end{itemize}
		\item[] \file{+ setAcList(acl: StringArray): void} \\		Metodo che permette di passare gli id degli amministratori abilitati/disabilitati per la \file{Rule};\\
		Parametri:
		\begin{itemize}
		...
		\item[] \file{+ getAcMode(): int} \\		Metodo che permette di ottenere l'\file{ac\_mode} della \file{Rule};\\
		\item[] \file{+ getAcList(): StringArray} \\		Metodo che permette di ottenere la lista degli id degli amministratori abilitati/disabilitati della \file{Rule};\\
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaUserListEvent.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassUserListQueryParameters.png}
...
\begin{itemize}
  ...
  \item 4: payload, prevede la presenza del campo \file{payload}.
\end{itemize};
...
\end{figure};
	...
	\begin{itemize}
		...
		Attributo che contiene l'elemento del DOM al quale verrà appesa la view dell'applicazione. Per informazoni sul tipo \file{HTMLElement} fare riferimento alla pagina \url{https://developer.mozilla.org/en/docs/Web/API/HTMLElement}  (visitato in data 2017-03-21);
...
\item l'esecuzione di richieste HTTP sostituendo le funzioni di Callback fornite da Javascript con le Promise che sono più chiare in casi di annidamento;
\end{itemize}
...
\subsubsection{IBMWatson}
	SDK node.js per l'accesso ai servizi cloud IBM Watson.
\paragraph{Classi}
\hypertarget{SpeechToTextV1_label}{\subparagraph{SpeechToTextV1}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassSpeechToTextV1.png}
	\caption{IBMWatson::SpeechToTextV1}
\end{figure}
\begin{itemize}
...
Per la relativa documentazione, consultare la pagina \url{https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/#recognize_audio_websockets}  (visitato in data 2017-03-21);
	\item \textbf{Attributi}:
	\begin{itemize}
...
\item narrowband.
\end{itemize}
Si rimanda alla relativa documentazione (\url{https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/?curl#get_models}  (visitato in data 2017-03-21)) per ulteriori chiarimenti;
...
\subsubsection{MicrosoftSpeakerRecognition}
Pkgs lib esterna
\paragraph{Classi}
\subsubsection{React}
Package per react
\paragraph{Classi}
\hypertarget{Component {abstract}_label}{\subparagraph{Component {abstract}}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassComponent {abstract}.png}
	\caption{React::Component {abstract}}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{Component {abstract}};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe astratta;
	\item \textbf{Utilizzo}: ;
	\item \textbf{Figlio}: \file{ConversationView};
\end{itemize}
\FloatBarrier

\subsubsection{RxJS 5}
Libreria esterna
\paragraph{Classi}
\hypertarget{Observable_label}{\subparagraph{Observable}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassObservable.png}
	\caption{RxJS 5::Observable}
\end{figure}
\begin{itemize}
	...
	\item \textbf{Utilizzo}: ;
	\item \textbf{Figli}: \file{UserObservable}, \file{ConversationObservable}, \file{GuestObservable}, \file{TaskObservable}, \file{RuleObservable}, \file{AgentObservable}, \file{ErrorObservable}, \file{MemberObservable};
\end{itemize}
\FloatBarrier

\hypertarget{Observer_label}{\subparagraph{Observer}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassObserver.png}
	\caption{RxJS 5::Observer}
\end{figure}
\begin{itemize}
...
\hypertarget{Subject_label}{\subparagraph{Subject}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassSubject.png}
	\caption{RxJS 5::Subject}
\end{figure}
\begin{itemize}
...
\paragraph{Classi}
\hypertarget{WebClient_label}{\subparagraph{WebClient}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassWebClient.png}
	\caption{Slack::WebClient}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{WebClient};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe rappresenta il modulo node.js che si occupa di comunicare con le API Web di Slack;
	\item \textbf{Utilizzo}: fornisce i metodi necessari ad usufruire delle funzionalità offerte dalle API Web di slack. Per ulterioir informazioni, fare riferimento alla documentazione dei slack (\url{https://api.slack.com/}  (visitato in data 2017-03-21));
\end{itemize}
\FloatBarrier

\subsubsection{WebSpeechApi}
Libreria esterna
\paragraph{Classi}
\hypertarget{SpeechSyntesis_label}{\subparagraph{SpeechSyntesis}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassSpeechSyntesis.png}
	\caption{WebSpeechApi::SpeechSyntesis}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{SpeechSyntesis};
	\item \textbf{Tipo}: \file{Class};
...
\hypertarget{SpeechSynthesisVoice_label}{\subparagraph{SpeechSynthesisVoice}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassSpeechSynthesisVoice.png}
	\caption{WebSpeechApi::SpeechSynthesisVoice}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{SpeechSynthesisVoice};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: ;
	\item \textbf{Utilizzo}: ;
\end{itemize}
