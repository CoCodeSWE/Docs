...
Ogni \gl{microservizio} rende disponibili degli \gl{endpoint}s, tramite i quali è possibile lo scambio di dati. \\
Per poter creare, pubblicare, mantenere e monitorare le \gl{API} che consentono di usufruire delle funzionalità supportate dal back-end, è necessario realizzare un \gl{API \gl{Gateway}} che si interpone tra client e back-end, permettendo al primo di comunicare con il secondo.
...
\begin{lstlisting}[language=json,firstnumber=1]
{
 rule_array : [
 {
  ...
  "\gl{task}":{ /* oggetto di tipo Back-end::Rules::Rule\gl{Task}Instance*/
   "params":{
	"par1":"Object",
	"par2":"Object",
	...
...
\item \file{user.add\gl{Enrollment}}: utilizza il metodo privato \file{addUserEnrollment(enr: Enrollment)} per aggiungere un Enrollment ad un utente registrato;
