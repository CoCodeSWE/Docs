...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi relativi ad una richiesta all'API REST del back-end e viene utilizzata dalla classe \file{VARequestAPIEvent}. FORSE DA SPOSTARE NEL CLIENT;
	\item \textbf{Attributi}:
	\begin{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/Class> MembersDAO.png}
...
\end{itemize};
	...
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
		\item OUT \hyperlink{Purpose_label}{\file{Purpose}}
		\item OUT \hyperlink{Topic_label}{\file{Topic}}
	\end{itemize}
\end{itemize}
\FloatBarrier
...
Per la relativa documentazione, consultare la seguente pagina \url{https://api.slack.com/methods/channels.setPurpose};
	...
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
		\item IN \hyperlink{NotificationChannel_label}{\file{NotificationChannel}}
	\end{itemize}
\end{itemize}
\FloatBarrier
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/Class>VocalLoginModule.png}
...
È soggetta ad una constructor-based dependency injection, la quale ha come oggetto un \file{Agent} relativo alle richieste da inviare;
	...
	\begin{itemize}
		...
		Attributo contenente il campo \file{versioning} da inviare in una richiesta ad api.ai. Per chiarire quanto appena detto, consultare la pagina \url{https://docs.api.ai/docs/versioning};
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa si occupa di notificare gli \file{Observer} collegati dell'avvenimento di un evento;
	...
	\begin{itemize}
		\item[] \file{+ next(): void} \\
		Metodo che notifica gli \file{Observer} collegati dell'avvenimento di un determinato evento;\\
		\item[] \file{+ error(err: Error): void} \\
		Metodo che permette di notificare gli \file{Observer} collegati nel caso si verifichi un errore;\\
		Parametri:
		\begin{itemize}
			\item \file{err: Error} \\
		...
		Metodo che permette di ottenere un \file{ErrorObservable} collegata a questo subject, evitando di doverlo esporre;\\
	\end{itemize}
\end{itemize}
\FloatBarrier
