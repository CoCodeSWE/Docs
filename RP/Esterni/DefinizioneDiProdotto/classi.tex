...
\hypertarget{PausableObserver_label}{\paragraph{PausableObserver}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassPausableObserver.png}
	\caption{Client::PausableObserver}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{PausableObserver};
	\item \textbf{Tipo}: \file{Abstract Class};
	\item \textbf{Descrizione}: questa classe astratta fornisce i metodi che permettono di mettere in pausa un Observer. Questa classe implementa l'interfaccia della libreria esterna RxJS::Observer;
	\item \textbf{Utilizzo}: le classi PlayerObserver, ApplicationManagerObserver e LogicObserver derivano da essa e ne definiscono i metodi astratti secondo le rispettive necessità;
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ next(data: Event): void} \\
		...
		Parametri:
		\begin{itemize}
			\item \file{data: Event} \\
			Parametro contenente i dati da passare alla funzione \_next;
		\end{itemize}
		\item[] \file{+ \_next(data: Event): void {abstract}} \\
...
\begin{figure}[h]
	...
	\caption{Client::Audio::PlayerObserver}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{PlayerObserver};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di inviare l'audio contenente la risposta fornita dal sistema al Player. È la classe che funziona da Observer;
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'audio relativo alla risposta fornita dal sistema;
	\item \textbf{Padre}: \file{PausableObserver};
	\item \textbf{Metodi}:
	\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::SpeechEndObservable}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{SpeechEndObservable};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che rappresenta un \file{Observable} che notifica gli obsrver inteessati quando l'ospite ha finito di parlare;
	\item \textbf{Utilizzo}: fornisce il meccanismo che permette agli observer di iscriversi per essere notificati quando l'ospite finisce di parlare. Viene creata a partire dal subject, per evitare di dover condividere quest'ultimo;
	\item \textbf{Metodi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- ui: Element} \\
		Attributo contenente l'\file{Element} dell'interfaccia dell'applicazione. Per informazioni sul tipo \file{Element}, fare riferimento alla pagina seguente:\url{https://developer.mozilla.org/en-US/docs/Web/API/Element};
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createApplication(pkg: ApplicationPackage): Application} \\
		Metodo che permette di costruire un Application a partire da un ApplicationPackage. \\ Esegue il codice presente all'interno del campo \file{setup} di \file{pkg}, eseguendo il binding di this per far sì che tale codice abbia accesso all'oggetto che sta venendo creato, quindi crea il metodo \file{runCmd} a partire dal codice presente all'interno di \file{pkg.cmdHandler}, eseguendo nuovamente il binding di this;\\
		Parametri:
		\begin{itemize}
			\item \file{pkg: ApplicationPackage} \\
			Package contenente i dati relativi all'applicazione da istanziare;
		\end{itemize}
		\item[] \file{+ runCmd(cmd: String, params: Array): void} \\
		...
		\end{itemize}
		\item[] \file{+ getUI(): Element} \\
		Metodo che permette di restituire l'interfaccia dell'applicazione da eseguire, sotto forma di un oggetto di tipo \file{Element} (\url{https://developer.mozilla.org/en-US/docs/Web/API/Element}). Questo \file{Element} verrà poi appeso al proprio frame da Manager;\\
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce all'\file{ApplicationManager} un meccanismo per cambiare l'applicazione in esecuzione. Questo avviene salvando lo stato dell'applicazione corrente nello \file{State} e recuperando la nuova applicazione da \file{State} (se presente) o dal \file{ApplicationRegistryClient}.\\ ------------- ELIMINEREI TUTTO QUELLO CHE C'È QUI SOTTO, LE DESCRIZIONI DEI METODI SONO NEI METODI APPUNTO---------- I seguenti metodi rendono questa classe soggetta ad una setter-based dependency injection: \begin{itemize} \item \file{setFrame}, che ha come oggetto un \file{HTMLElement} contenente ciò che racchiude l'interfaccia dell'applicazione in esecuzione; \item \file{setObservable}, che ha come oggetto un \file{DataArrivedObservable} notificante dell'arrivo di nuovi dati dall'API Gateway; IN REALTÀ QUESTO ANCHE NO ??? ??? ??? ??? ??? ??? ??? ??? ??? ??? ??? ??? \item \file{setRegistryClient}, che ha come oggetto un ApplicationRegistryClient; \item \file{setElement} \end{itemize};
	\item \textbf{Attributi}:
	\begin{itemize}
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createManager(rc: ApplicationRegistryClient, frame: Element): Manager} \\
		...
		\begin{itemize}
			...
			\item \file{frame: Element} \\
			Parametro che permette la dependency injection di \file{Element} all'interno di \file{Manager}. Tale oggetto rappresenta l'elemento del DOM al quale verrà appesa l'interfaccia dell'applicazione;
		\end{itemize}
		\item[] \file{+ runApplication(app: String, cmd: String, params: Array): void} \\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassApplicationRegistryClient.png}
...
\begin{itemize}
	...
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- client: IRegistryClient} \\
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ <<Create>> createApplicationRegistryAdapter(client: IRegistryClient): ApplicationRegistryAdapter} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce un meccanismo che permette l'invio dei dati, il testo della risposta dell'assistente virtuale, una volta che questi ultimi sono stati forniti dal (sistema). Gli observer collegati sono: \begin{itemize} \item ApplicationManagerObserver; \item PlayerObserver. \end{itemize};
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce dei meccanismi per: \begin{itemize} \item la comunicazione con l'API gateway; \item pubblicare eventi. \end{itemize};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: zzz;
	\item \textbf{Utilizzo}: zzz;
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\begin{lstlisting}[language=json,firstnumber=1]
	{
		"Records": [ {
			...
			"Subject":"oggetto del messaggio", "Timestamp":"2017-03-26T20:39:48.599Z"
			}
		 } ]
	} \end{lstlisting};
	\item \textbf{Attributi}:
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassGuestsDAO.png}
	\caption{Back-end::GuestsDAO}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{GuestsDAO};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di astrarre le modalità d'interazione al database contenente gli ospiti conosciuti;
	\item \textbf{Utilizzo}: fornisce a CHICOSAAAAAAAAAAAA? un meccanismo per accedere al database contenente i dati relativi agli ospiti conosciuti, senza conoscerne le modalità di implementazioni e di persistenza di quest'ultimo. Permette operazioni di lettura, scrittura e rimozione di ospiti conosciuti;
	\item \textbf{Attributi}:
	\begin{itemize}
	...
	\begin{itemize}
		...
		Metodo che permette di ottenere un ospite a partire dal nome e l'azienda di provenienza. \\ L'\file{Observable} restituito riceverà l'oggetto rappresentante tale \file{Guest}, e verrà completato. Nel caso in cui l'ospite richiesto non sia presente nel database, gli \file{Observer} interessati non riceveranno alcun valore, ma verranno notificati tramite la chiamata del loro metodo \file{error()};\\
		Parametri:
		\begin{itemize}
			\item \file{name: String} \\
...
\begin{itemize}
	...
	\item \textbf{Padre}: \file{WebhookService <<interface>>};
	\item \textbf{Attributi}:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{lstlisting} Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response};
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dal webhook per inviare la risposta. La risposta, contenuta nel \file{LambdaResponse} parametro del metodo \file{LambdaContext::succeed}, possiede un attributo \file{body}, il quale conterrà il corpo di essa sotto forma di una stringa in formato JSON, organizzando i dati nel seguente modo:
			\begin{lstlisting}[language=json,firstnumber=1]
			\end{lstlisting} ;
		\end{itemize}
	\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassConversationsDAO.png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce l'insieme dei parametri necessari ad identificare un audio, nel formato richiesto dall'API Watson Speech to Text, che verrà poi convertito in testo. Per la relativa documentazione, consultare la pagina \url{https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/#recognize_audio_websockets};
	\item \textbf{Attributi}:
	\begin{itemize}
		...
		Attributo contenente il tempo in secondi dopo il quale, se nell'audio viene rilevato solo del silenzio, la connessione deve essere chiusa. Il valore di default di questo attributo è 30 secondi. \\ Per non chiudere mai la connessione, si può impostare questo attributo a -1;
		\item[] \file{+ model: String[0..1]} \\
		Attributo contenente il nome del model relativo all'audio da trascrivere.\\ Un model è un parametro che indica la lingua e il tipo di sampling rates usato per essa. Il tipo del sampling rates supportato può assumere uno tra i seguenti valori: \begin{itemize} \item broadband; \item narrowband. \end{itemize} Si rimanda alla relativa documentazione (\url{https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/?curl#get_models}) per ulteriori chiarimenti;
		\item[] \file{+ interim\_results: Boolean[0..1]} \\
		Attributo contenente un valore booleano che indica se posso essere ritornati risultati parziali o meno. Se impostato a true, i risultati parziali sono ritornati come uno stream di oggetti JSON ed ognuno di essi rappresenta un singolo SpeechRecognitionEvent. Se impostato a false, verrà ritornato un unico SpeechRecognitionEvent contenente il risultato finale. \\ Questro attributo ha valore di default uguale a false;
		...
		Attributo contenente un valore di confidenza, contenuto nell'intervallo [0,1], che è il limite inferiore per una keyword trovata. Una parola fa match in una keyword se la sua confidenza è maggiore o uguale al valore di questo attributo. \\ Se questo parametro è omesso, nessuna keyword sarà trovata, altrimenti deve essere fornita almeno una keyword;
		\item[] \file{+ word\_alternatives\_threshold: Real[0..1]} \\
		Attributo contenente un valore di confindenza, contenuto nell'intervallo [0,1], che è il limite inferiore per identificare un'ipotetica parola come possibile alternativa. \\ Una parola alternativa è considerata tale se la sua confidenza è maggiore o uguale al valore di questo attributo. Se questo parametro è omesso, nessuna parola alternativa verrà fornita;
		\item[] \file{+ profanity\_filter: Boolean[0..1]} \\
		Attributo contenente un valore booleano che indica se il profanity filter verrà applicato al testo trascritto. \\ Il profanity filter è un meccanismo che sostituisce parole inappropriate con degli asterischi. Questo filtro può essere applicato solo a trascrizioni in lingua US English. \\ Il valore di default per questo attributo è uguale a true;
		\item[] \file{+ smart\_formatting: Boolean[0..1]} \\
		Attributo contenente un valore booleano che indica se date, orari, serie di numeri e cifre, numeri di telefono, valori monetari e indirizzi Internet devono essere convertiti, nella trascrizione finale, in un formato più leggibile. Questo meccanismo può essere applicato solo a trascrizioni in lingua US English. \\ Il valore di default per questo attributo è uguale a false;
		...
		Indicates whether labels that identify which words were spoken by which participants in a multi-person exchange are to be included in the response. If true, speaker labels are returned; if false (the default), they are not. Speaker labels can be returned only for the following language models: en-US\_NarrowbandModel es-ES\_NarrowbandModel ja-JP\_NarrowbandModel Setting speaker\_labels to true forces the continuous and timestamps parameters to be true, as well, regardless of whether the user specifies false for the parameters. For more information, see Speaker labels;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaWebhookResponse_label}{\paragraph{LambdaWebhookResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaWebhookResponse.png}
	\caption{Back-end::LambdaWebhookResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaWebhookResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{WebhookResponseBody}. DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE;
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un \file{WebhookResponseBody} come risposta;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: Webhook} \\
		Attributo contenente;
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di rappresentare e organizzare gli attributi relativi ad un ospite conosciuto, i quali dovranno essere salvati database;
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ webhook(event: LambdaEvent, context: LambdaWebhookContext): void} \\
		...
		\begin{itemize}
			...
			Parametro contenente;
			\item \file{context: LambdaWebhookContext} \\
			Parametro contenente;
		\end{itemize}
...
\hypertarget{FunctionObserver_label}{\paragraph{FunctionObserver}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassFunctionObserver.png}
	\caption{Back-end::FunctionObserver}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{FunctionObserver};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che rappresenta un \file{Observer} che si aspetta dati di tipo \file{Function};
	\item \textbf{Utilizzo}: implementa il metodo \file{next()} dell'interfaccia, in maniera tale che accetti dati di tipo \file{Function};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ next(function: Function): void} \\
		Metodo che permette agli \file{Observable} di notificare l'\file{Observer} con dati di tipo \file{Function}. Definisce inoltre le operazioni che l'\file{Observer} compierà all'arrivo di tali dati;\\
		Parametri:
		\begin{itemize}
			\item \file{function: Function} \\
			Parametro contenente la \file{Function} mandata dall'\file{Observable};
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{FunctionObservable_label}{\paragraph{FunctionObservable}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassFunctionObservable.png}
	\caption{Back-end::FunctionObservable}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{FunctionObservable};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe implementa un \file{Observable} che permette l'iscrizione di \file{FunctionObserver};
	\item \textbf{Utilizzo}: fornisce i meccanismi necessari per il passaggio di una serie di \file{Function} ad un \file{Observer} interessato;
	\item \textbf{Padre}: \file{Observable};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createFunctionObservable(onSubscription: function(observer: FunctionObserver) : void): FunctionObservable} \\
		Constructor di \file{FunctionObservable};\\
		Parametri:
		\begin{itemize}
			\item \file{onSubscription: function(observer: FunctionObserver) : void} \\
			Funzione che verrà eseguita quando un \file{Observer} si iscrive all'\file{Observable}. Si occupa di passare i dati all'\file{Observer}, chiamando il metodo \file{next(function: Function)}. Quando non ci sono più dati da restituire, si occupa di chiamare il metodo \file{complete()}. Nel caso in cui si verificasse un errore, si occupa di chiamare il metodo \file{error(err: Object)} con i dati relativi all'errore verificatosi;
		\end{itemize}
		\item[] \file{+ subscribe(observer: FunctionObserver): void} \\
		Metodo che permette ad un \file{FunctionObserver} interessato di iscriversi a questo \file{Observable};\\
		Parametri:
		\begin{itemize}
			\item \file{observer: FunctionObserver} \\
			\file{Observer} che si vuole iscrivere;
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce il metodo query, che viene utilizzato dall'unico endpoint necessario al client vocale. --- Fornisce un meccanismo che: \begin{itemize} \item permette di dedurre, tramite i microservizi di STT e di assistente virtuale, il servizio necessario al client vocale, utilizzando il metodo \file{queryLambda}; \item permette al client vocale di usufruire delle funzionalità, supportate dal \file{Backend}, tramite i metodi privati che questa classe fornisce. \end{itemize};
	\item \textbf{Attributi}:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{lstlisting} Per la relativa documentazione, consultare la pagina \url{https://www.ibm.com/watson/developercloud/speech-to-text/api/v1/#recognize\_sessionless\_nonmp12};
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi relativi ad un Enrollment e viene utilizzata come parametro dei metodi \file{AuthAPI::addEnrollment(event: Enrollment, context: LambdaEnrollmentContext)} e \file{AuthAPI::login(event: Enrollment, context: LambdaTokenContext)}. CAMBIARE IL TIPO DEI PARAMETRI DEI METODI QUI SOPRA ??? CAMBIARE IL TIPO DEI PARAMETRI DEI METODI QUI SOPRA CAMBIARE IL TIPO DEI PARAMETRI DEI METODI QUI SOPRA;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ username: String} \\
...
\hypertarget{RawEvent_label}{\paragraph{RawEvent}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassRawEvent.png}
	\caption{Back-end::APIGateway::RawEvent}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{RawEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di permettere l'invio alle lambda function di dati non interpretati;
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio di dati non interpretati da API Gateway alle lambda function. Il corpo della richiesta ricevuta da API Gateway è presente nell'attributo \file{body}, codificato in Base64. DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: String} \\
		Attributo contenente il corpo della richiesta ricevuta da API Gateway, codificato in Base64;
		\item[] \file{+ isBase64Encoded: boolean} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce i metodi getter e setter per i parametri relativi ad un utente registrato, i quali dovranno essere memorizzati nel database per questo microservizio.\\ È utilizzata dalla classe \file{UsersDAO} e dalle classi che utilizzano quest'ultima;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- username: String} \\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassUsersDAO.png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce a \file{UsersService} un meccanismo per accedere al database contenente gli utenti registrati, senza conoscerne le modalità di implementazione e di persistenza di quest'ultimo. A partire da un identificativo, permette operazioni di lettura, scrittura e rimozione di utenti registrati;
	\item \textbf{Figlio}: \file{UsersDAODynamoDB};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ addUser(user: User): ErrorObservable} \\
		Metodo che permette di aggiungere un utente. \\ L'\file{Observable} restituito non riceverà alcun valore, ma verrà completato in caso di aggiunta dell'utente avvenuta con successo. In caso di errore durante l'aggiunta dell'utente, gli \file{Observer} interessati verranno notificati tramite la chiamata del loro metodo \file{error()} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{user: User} \\
		...
		Metodo che permette di ottenere i dati relativi ad un utente. \\ L'\file{Observable} restituito riceverà l'oggetto rappresentante tale \file{User}, e verrà completato. Nel caso in cui l'utente richiesto non sia presente nel database, gli \file{Observer} interessati non riceveranno alcun valore, ma verranno notificati tramite la chiamata del loro metodo \file{error()};\\
		Parametri:
		\begin{itemize}
			\item \file{username: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce l'insieme dei parametri necessari ad identificare, tramite SR, un utente registrato e viene utilizzata in \file{VocalLoginModule::getUser(id: String)} e \file{VocalLoginModule::getList()}. CI VUOLE IL LINK ??? CI VUOLE IL LINK ??? CI VUOLE IL LINK ??? CI VUOLE IL LINK ???;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ id: String} \\
		...
		Attributo contenente lo stato dell'Enrollment.\\ Lo stato può essere uno tra i seguenti valori: \begin{itemize} \item Enrolling: indica che la fase di enrollment è in corso; \item Training: indica che il microservizio di Speaker Recognition sta elaborando e organizzando i dati ricevuti (analizza le frasi comunicate e ne costruisce un'impronta vocale); \item Enrolled, ovvero che le due fasi precedenti sono state completate. \end{itemize};
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce i metodi che implementano le lambda function necessarie alla gestione degli utenti. Questa classe non interagisce direttamente con il database, ma fa utilizzo di \file{UsersDAO}, il quale nasconde i meccanismi di accesso e persistenza dei dati nel database;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- users: UsersDAO} \\
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di realizzare e raggruppare tutte le operazioni necessarie all'identificazione di un utente registrato, tramite il microservizio di Speaker Recognition (SR). MANCA IL DAO SUGLI USER?CHI USA QUESTA CLASSE?;
	\item \textbf{Utilizzo}: fornisce un meccanismo per creare, eliminare ed ottenere un utente registrato associato ad un Enrollment, il quale è necessario all'identificazione di un utente tramite il microservizio di Speak Recognition (SR). Permette quindi di associare i parametri relativi ad un \file{Enrollment} ad un utente registrato.\\ È soggetta ad una constructor-based dependency injection, la quale ha come oggetto un \file{VocalLoginModulConfig};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- min\_confidence: int} \\
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ addEnrollment(id: String, audio: Blob): } \\
...
\hypertarget{LambdaUserContext_label}{\paragraph{LambdaUserContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaUserContext.png}
	\caption{Back-end::Auth::LambdaUserContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaUserContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{User};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio di un \file{User} presente nel sistema da API Gateway alle lambda function;
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ succed(user:  LambdaUserResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{user:  LambdaUserResponse} \\
			Parametro contenente;
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{LambdaUserResponse_label}{\paragraph{LambdaUserResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaUserResponse.png}
	\caption{Back-end::Auth::LambdaUserResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaUserResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{User};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un \file{User} come risposta;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: User} \\
		Attributo contenente;
...
\hypertarget{LambdaUserArrayResponse_label}{\paragraph{LambdaUserArrayResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaUserArrayResponse.png}
	\caption{Back-end::Auth::LambdaUserArrayResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaUserArrayResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{UserArray};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un array di \file{User} come risposta;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: UserArray} \\
		Attributo contenente l'array di \file{User} che si vuole inviare come risposta;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaUserArrayContext_label}{\paragraph{LambdaUserArrayContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaUserArrayContext.png}
	\caption{Back-end::Auth::LambdaUserArrayContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaUserArrayContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{UserArray};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutti gli \file{User} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ succeed(users: LambdaUserArrayResponse): void} \\
		Metodo che permette alle lambda function di inviare una risposta contenente un array di \file{User};\\
		Parametri:
		\begin{itemize}
			\item \file{users: LambdaUserArrayResponse} \\
			Parametro contente la lista degli utenti da restituire;
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Padre}: \file{UsersDAO};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- db: AWS::DynamoDB} \\
		Attributo contenente un riferimento al modulo di Node.js utilizzato per l'accesso al database DynamoDB contenente la tabella degli utenti;
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassAgentsDAO.png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un Agent che dovranno essere memorizzati nel database per questo microservizio. In api.ai, un Agent ha lo scopo di trasformare il linguaggio naturale, ricevuto in input, in dati capibili per le applicazioni. Per la relativa documentazione, consultare questa pagina \url{https://docs.api.ai/docs/concept-agents};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
\hypertarget{VAModule _label}{\paragraph{VAModule }}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassVAModule .png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce la definizione dei metodi che dovranno essere implementati, in maniera tale da permettere ad un modulo che si interfacci con un assistente virtuale di essere utilizzato da \file{VAService}. --- Fornisce la definizione dei metodi che dovranno essere implementati, in maniera tale da permettere ad un modulo che si interfacci con api.ai di essere utilizzato da \file{VAService};
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di accedere alle api dell'assistente virtuale api.ai implementando l'interfaccia VAModule. --- Questa classe si occupa di convertire l'interfaccia fornita da api.ai in una più adatta alle esigenze dell'applicazione, definita da \file{VAModule}. \\ Facendo da Adapter tra le API di api.ai (adaptee) e l'interfaccia \file{VAModule} (target) utilizzata da \file{VAService}, permette l'interoperabilità tra queste due interfacce;
	\item \textbf{Utilizzo}: questa classe permette di interrogare le API di api.ai, ottenendo una risposta. Riceve una dependency injection di Agent dal costruttore, in modo da sapere a quale Agent di api.ai devono essere mandate le richieste. Per fare le richieste HTTP alle API il metodo \file{query} utilizza il modulo request-promise. --- Fornisce a \file{VAModule} un meccanismo che permette di interrogare le API di api.ai, in modo da consentire a \file{VAService} l'utilizzo di quest'ultime utilizzando un'interfaccia distinta e più consona alle proprie esigenze. \\ È soggetta ad una constructor-based dependency injection, la quale ha come oggetto un \file{Agent} relativo alle richieste da inviare;
	\item \textbf{Padre}: \file{VAModule <<interface>>};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- agent: Agent} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi necessari per rappresentare il corpo della risposta dell'assistente virtuale. \\ Viene passata come parametro al metodo \file{cmdHandler} delle varie applicazioni, il quale si occupa di estrarre i parametri necessari ad eseguire il comando ricevuto. DOV'È IL METODO cmdHandler ??? DOV'È IL METODO cmdHandler ??? DOV'È IL METODO cmdHandler ??? DOV'È IL METODO cmdHandler ??? DOV'È IL METODO cmdHandler ???;
	\item \textbf{Attributi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo contenente il \file{VAModule}. VAModule è un'interfaccia, implementata da ApiAiVAAdapter. Non dovremmo dire che vengono contattati i metodi implementati in quest'ultima classe ??? -----;
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		...
		\begin{itemize}
			...
			\end{lstlisting} con \file{app} stringa che corrisponde al nome dell'applicazione che manda la richiesta, e \file{query} oggetto del tipo \file{VAQuery} contenente i dati relativi alla query da mandare all'assistente virtuale;
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dalle lambda function per inviare la risposta. La risposta, contenuta nel \file{LambdaResponse} parametro del metodo \file{LambdaContext::succeed}, possiede un attributo \file{body}, il quale conterrà il corpo di essa sotto forma di una stringa in formato JSON, organizzando i dati nel seguente modo:
			\begin{lstlisting}[language=json,firstnumber=1]
			{
			...
			\end{lstlisting} Dove: \begin{itemize} \item \file{action}: attributo contenente l'azione che l'assistente virtuale richiede di compiere al client. La stringa è nel formato \file{[nome\\_applicazione.comando]}, dove \file{nome\\_applicazione} indica l'applicazione che deve essere eseguita, mentre \file{comando} indica il comando che il client deve far eseguire all'applicazione. \\ Ad esempio, \file{conversation.displayMsgs} comunica al client di far eseguire il comando \file{displayMsgs} all'applicazione che si occupa di mostrare la conversazione; \item \file{res}: attributo contenente il corpo della risposta ricevuta dall'assistente virtuale. Il contenuto di questo attributo sarà passato senza modifiche al metodo \file{Client::ApplicationManager::Application::runCmd} dell'applicazione.\\ In questo oggetto abbiamo i campi \file{contexts} (array contenente i context ricevuti dall'assistente virtuale), \file{data} (attributo che può essere utilizzato per scambiare dati tra client e l'eventuale webhook. Il suo contenuto dipende dal servizio webhook utilizzato. Il client si deve occupare di reinviare i dati presenti in questo campo di una determinata risposta nella richiesta successiva. In particolare, questo attributo viene utilizzato per lo scambio, tra client e back-end, del token dello agent in api.ai), \file{text\\_request} (attributo contenente il testo della richiesta dell'utente) e \file{text\\_response}(attributo che contiene il testo della risposta dell'assistente virtuale). \item \file{session\\_id}: attributo contenente l'id univoco della sessione corrente dell'assistente virtuale. \end{itemize};
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{VAQueryEvent_label}{\paragraph{VAQueryEvent}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassVAQueryEvent.png}
	\caption{Back-end::VirtualAssistant::VAQueryEvent}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{VAQueryEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: da fare. ???? DA ELIMINARE ??? DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???VDA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???DA ELIMINARE???;
	\item \textbf{Utilizzo}: da fare;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ app: String} \\
		Da fare;
		\item[] \file{+ query: VAQuery} \\
		Da fare;
	\end{itemize}
\end{itemize}

\hypertarget{WebhookService _label}{\paragraph{WebhookService }}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassWebhookService .png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce una serie di metodi necessari per definire un servizio che soddisfi i requisiti per webhook di api.ai. Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/webhook};
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \file{context: LambdaContext} \\
			Parametro contenente il contesto da utilizzare per inviare la risposta dal WebhookService ad api.ai. \\ La risposta mandata dovrà rispettare i requisiti per i servizi di webhook di api.ai;
		\end{itemize}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un Context. In api.ai, un Context è una stringa che rappresenta il contesto corrente nel quale un utente somministra una richiesta, in maniera tale da disambiguare il più possibile quest'ultime. \\ Per la relativa documentazione, consultare questa pagina \url{https://docs.api.ai/docs/concept-contexts};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
		...
		Attributo contenente il lifespan del Context. Questo valore indica per quanti altri matched intents bisogna mantenere il Context. Per chiarire quanto appena detto, consultare la relativa documentazione in questa pagina \url{https://api.ai/blog/2015/11/23/Contexts/};
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi per rappresentare l'oggetto buttons relativo ad una risposta. \\ Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/rich-messages};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ text: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi relativi ai dati di intents e contexts e viene utilizzato come attributo della classe \file{ProcessingResult}. Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response};
	\item \textbf{Attributi}:
	\begin{itemize}
		...
		Attributo contenente un valore booleano che indica se è stato usato un webhook for slot filling. Per chiarire quanto detto, consultare la relativa documentazione \url{https://docs.api.ai/docs/webhook#webhook-for-slot-filling};
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un MsgObject, il quale sarà contenuto in un \file{Fulfillment}. La classe \file{Fulfillment} fa utilizzo di un array di essi per adempire alla richiesta di un utente;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ type: int} \\
		Attributo contenente;
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un fulfillment. In api.ai un fulfillment è tutto ciò che viene ritornato dalla richiesta di utente. In particolare, è l'adempimento ad una richiesta e può comprendere diversi campi dati. Per chiarire quanto appena detto, consultare la relativa documentazione \url{https://docs.api.ai/docs/webhook};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\hypertarget{LambdaVAResponse_label}{\paragraph{LambdaVAResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaVAResponse.png}
	\caption{Back-end::VirtualAssistant::LambdaVAResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaVAResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{VA}. DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE;
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un \file{VA} come risposta;
	\item \textbf{Padre}: \file{LambdaResponse};
	\item \textbf{Figlio}: \file{LambdaEmptyResponse };
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: VAResponse} \\
		Attributo contenente;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaVAContext_label}{\paragraph{LambdaVAContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaVAContext.png}
	\caption{Back-end::VirtualAssistant::LambdaVAContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaVAContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{VA};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutti i \file{VA} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Padre}: \file{LambdaContext};
	\item \textbf{Figlio}: \file{LambdaEmptyContext};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaVAResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaVAResponse} \\
			Parametro contenente;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaVAResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaVAResponse} \\
			Parametro contenente;
		\end{itemize}
		\item[] \file{+ fail(res: LambdaVAResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaVAResponse} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce i campi necessari all'interrogazione di un assistente virtuale, utilizzando il nome di un evento e dei parametri al posto della stringa di query \file{VAQuery::text}. Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/concept-events};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un target di una Rule che dovranno essere memorizzati nel database per questo microservizio. Viene utilizzata dalla classe Rule per definire un Array di Target ai quali applicare la sua funzione;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ company: String} \\
...
\hypertarget{RuleFunctionInstance_label}{\paragraph{RuleFunctionInstance}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassRuleFunctionInstance.png}
	\caption{Back-end::Rules::RuleFunctionInstance}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{RuleFunctionInstance};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di rappresentare i dati relativi alla funzione di una direttiva;
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce i metodi getter e setter per i parametri relativi ad una direttiva, i quali dovranno essere memorizzati nel database per questo microservizio. Tramite il metodo \file{setFunction}, è soggetta ad una setter-based dependency injection che ha come oggetto una \file{RuleFunctionInstance}. \\ È utilizzata dalla classe \file{RulesDAO} e dalle classi che utilizzano quest'ultima;
	\item \textbf{Attributi}:
	\begin{itemize}
		...
		Attributo contenente0 per positivo, 1 per negativo;
		\item[] \file{- ac\_list: StringArray} \\
		Attributo contenente l'array di stringhe di id amministratori abilitati/disabilitati in base a valore di \file{ac_mode};
		\item[] \file{- enabled: boolean} \\
		Attributo contenente un valore che dice se la Rule è abilitata o meno;
		\item[] \file{- function: RuleFunctionInstance} \\
		Attributo contenente la funzione della Rule;
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> Rule(targ: RuleTargetArray, name: String, ac\_mode: int, ac\_list: StringArray, en: boolean, function: String): Rule} \\
		Metodo che permette di instanziare un oggetto \file{Rule} a partire da un nome, una lista dei targets un \file{ac\_mode}, un \file{ac\_list}, una funzione da applicare e un valore booleano per abilitarla o meno;\\
		Parametri:
		\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ setFunction(function: RuleFunctionInstance): void} \\
		Metodo che permette di passare la funzione da applicare per la \file{Rule};\\
		Parametri:
		\begin{itemize}
			\item \file{function: RuleFunctionInstance} \\
		...
		\item[] \file{+ getFunction(): void} \\
		Metodo che permette di ottenere la \file{Function} applicata dalla \file{Rule};\\
	\end{itemize}
\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassRulesDAO.png}
...
\hypertarget{Function_label}{\paragraph{Function}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassFunction.png}
	\caption{Back-end::Rules::Function}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{Function};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di rappresentare la funzione che dovrà essere applicata ad una \file{Rule};
	\item \textbf{Utilizzo}: fornisce gli attributi delle funzioni che dovranno essere applicate a certe \file{Rule}, i quali dovranno essere memorizzati nel database per questo microservizio. Inoltre, una sua istanza viene inserita nel body della classe \file{LambdaFunctionResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ function: String} \\
		Attributo contenente la funzione da applicare;
...
\hypertarget{FunctionsDAO_label}{\paragraph{FunctionsDAO}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassFunctionsDAO.png}
	\caption{Back-end::Rules::FunctionsDAO}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{FunctionsDAO};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa quella si occupa di astrarre le modalità d'accesso al database per questo microservizio, contenente le \file{Function};
	\item \textbf{Utilizzo}: fornisce a \file{RuleService} un meccanismo per accedere al database, contenente le funzioni da applicare a certe\file{Rule}, senza conoscerne le modalità di implementazioni e di persistenza del database. A partire da un identificativo, permette operazioni di lettura, scrittura e rimozione delle \file{Function};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ getFunctionList(): FunctionObservable} \\
		Metodo che permette di ottenere la lista delle funzioni. L'\file{Observable} restituito manderà agli \file{Observer} le funzioni ottenute, una alla volta, e poi chiama il loro metodo \file{complete}. Nel caso in cui si verifichi un errore, gli \file{Observer} iscritti verranno notificati tramite la chiamate del loro metodo \file{error} con i dati relativi all'errore verificatosi;\\
		\item[] \file{+ addFunction(fun: Function): ErrorObservable} \\
		Metodo che permette di aggiungere una funzione al database. \\ L'\file{Observable} restituito non riceverà alcun valore, ma verrà completato in caso di aggiunta della funzione avvenuta con successo. In caso di errore durante l'aggiunta della funzione, gli \file{Observer} interessati verranno notificati tramite la chiamata del loro metodo \file{error()} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{fun: Function} \\
			Parametro contenente la funzione;
		\end{itemize}
		\item[] \file{+ removeFunction(id: int): ErrorObservable} \\
		Metodo che permette di rimuovere una funzione dal DB a partire dal suo id. \\ L'\file{Observable} restituito non riceverà alcun valore, ma verrà completato in caso di aggiunta della funzione avvenuta con successo. In caso di errore durante l'aggiunta della funzione, gli \file{Observer} interessati verranno notificati tramite la chiamata del loro metodo \file{error()} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{id: int} \\
			Parametro contenente l'id della funzione;
		\end{itemize}
		\item[] \file{+ getFunction(id: int): ErrorObservable} \\
		...
		Metodo che permette di aggiornare una funzione. L'\file{Observable} restituito non riceverà alcun valore, ma verrà completato in caso di aggiunta della funzione avvenuta con successo. In caso di errore durante l'aggiunta della funzione, gli \file{Observer} interessati verranno notificati tramite la chiamata del loro metodo \file{error()} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{fun: Function} \\
			Parametro contenente la funzione;
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce un modulo di Node.js che esporta i metodi, i quali verranno utilizzati come lambda function per gli endpoint del microservizio delle direttive. Utilizza i moduli \file{RulesDAO} e \file{FunctionsDAO} per l'accesso ai dati relativi alle direttive ed alle loro funzioni. --- Fornisce i metodi che implementano le lambda function necessarie alla gestione delle \file{Rule} e relative \file{Function}. Questa classe non interagisce direttamente con il database, ma fa utilizzo di \flle{FunctionsDAO} e file{RulesDAO}, le quali nascondono i meccanismi di accesso e persistenza dei dati nel database;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- rules: RulesDAO} \\
		Attributo che permette di contattare il \file{RulesDAO}, il quale permette l'accesso al database delle \file{Rule};
		\item[] \file{- functions: FunctionsDAO} \\
		Attributo che permette di contattare il \file{FunctionsDAO}, il quale permette di accedere al database delle \file{Function};
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getFunctionList(event: LambdaEvent, context: LambdaContext): void} \\
		Metodo che implementa la lambda function che si occupa ottenere l'array delle \file{Function} disponibili;\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaEvent} \\
			Parametro contenente, all'interno del campo body, una stringa vuota;
		...
		\end{itemize}
		\item[] \file{+ getFunction(event: LambdaIdEvent, context: LambdaContext): void} \\
		Metodo che implementa la lambda function che si occupa di ottenere una \file{Function};\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaIdEvent} \\
			Parametro contenente, all'interno del campo \file{pathParameters}, l'identificativo della \file{Rule} della quale si vuole ottenere la \file{function};
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dalle lambda function per inviare la risposta. Il \file{body} del \file{LambdaResponse}, parametro del metodo \file{LambdaContext::succeed}, conterrà, sotto forma di una stringa in formato JSON, un oggetto di tipo \file{RuleFunctionInstance}, contenente i dati relativi alla \file{Function} ritornata;
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{LambdaRuleContext_label}{\paragraph{LambdaRuleContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaRuleContext.png}
	\caption{Back-end::Rules::LambdaRuleContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaRuleContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{Rule};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio di una \file{Rule} presente nel sistema da API Gateway alle lambda function;
	\item \textbf{Padre}: \file{LambdaContext};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaRuleResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaRuleResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ fail(res: LambdaRuleResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleResponse} \\
...
\hypertarget{LambdaRuleListContext_label}{\paragraph{LambdaRuleListContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaRuleListContext.png}
	\caption{Back-end::Rules::LambdaRuleListContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaRuleListContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{RuleArray};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutte le \file{Rule} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Padre}: \file{LambdaContext};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaRuleListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleListResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaRuleListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleListResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ fail(res: LambdaRuleListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaRuleListResponse} \\
...
\hypertarget{LambdaFunctionListContext_label}{\paragraph{LambdaFunctionListContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaFunctionListContext.png}
	\caption{Back-end::Rules::LambdaFunctionListContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaFunctionListContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{FunctionTypeArray};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutti le \file{FunctionType} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Padre}: \file{LambdaContext};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaFunctionListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaFunctionListResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaFunctionListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaFunctionListResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ fail(res: LambdaFunctionListResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaFunctionListResponse} \\
...
\hypertarget{LambdaFunctionContext_label}{\paragraph{LambdaFunctionContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaFunctionContext.png}
	\caption{Back-end::Rules::LambdaFunctionContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaFunctionContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{Function};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio di una \file{Function} presente nel sistema da API Gateway alle lambda function;
	\item \textbf{Padre}: \file{LambdaContext};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaFunctionResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaFunctionResponse} \\
			???;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaFunctionResponse): void} \\
		???;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaFunctionResponse} \\
...
\hypertarget{LambdaFunctionResponse_label}{\paragraph{LambdaFunctionResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaFunctionResponse.png}
	\caption{Back-end::Rules::LambdaFunctionResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaFunctionResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{FunctionType};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare una \file{FunctionType} come risposta;
	\item \textbf{Padre}: \file{LambdaResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: Function} \\
		???;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaFunctionListResponse_label}{\paragraph{LambdaFunctionListResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaFunctionListResponse.png}
	\caption{Back-end::Rules::LambdaFunctionListResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaFunctionListResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{FunctionTypeArray};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un array di \file{FunctionType} come risposta;
	\item \textbf{Padre}: \file{LambdaResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: FunctionArray} \\
		???;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaRuleListResponse_label}{\paragraph{LambdaRuleListResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaRuleListResponse.png}
	\caption{Back-end::Rules::LambdaRuleListResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaRuleListResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{RuleArray};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un array di \file{Rule} come risposta;
	\item \textbf{Padre}: \file{LambdaResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: RuleArray} \\
		???;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaRuleResponse_label}{\paragraph{LambdaRuleResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaRuleResponse.png}
	\caption{Back-end::Rules::LambdaRuleResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaRuleResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{Rule};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare una \file{Rule} come risposta;
	\item \textbf{Padre}: \file{LambdaResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: Rule} \\
...
\hypertarget{RuleEvent_label}{\paragraph{RuleEvent}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassRuleEvent.png}
	\caption{Back-end::Rules::RuleEvent}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{RuleEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che viene utilizzata per descrivere le Rules che vengono passate dall'API Gateway alle lambda function. DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE;
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio di dati grezzi da API Gateway alle lambda function;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ ac\_list: StringArray} \\
		Attributo contenente;
...
\hypertarget{FunctionsDAODynamoDB_label}{\paragraph{FunctionsDAODynamoDB}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassFunctionsDAODynamoDB.png}
	\caption{Back-end::Rules::FunctionsDAODynamoDB}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{FunctionsDAODynamoDB};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che si occupa di implementare l'interfaccia \file{FunctionsDAO}, utilizzando un database DynamoDB come supporto per la memorizzazione dei dati;
	\item \textbf{Utilizzo}: implementa i metodi dell'interfaccia \file{FUcntionsDAO} interrogando un database DynamoDB. Utilizza \file{AWS::DynamoDB::DocumentClient} per l'accesso al database. La dependency injection dell'oggetto \file{AWS::DynamoDB} viene fatta utilizzando il costruttore;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- db: AWS::DynamoDB} \\
		Attributo contenente un riferimento al modulo di Node.js utilizzato per l'accesso al database DynamoDB contenente la tabella degli utenti;
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Parametro contenente la funzione;
		\end{itemize}
		\item[] \file{+ getFunction(id: String): FunctionObservable} \\
		...
		\begin{itemize}
			...
			Parametro contenente la funzione;
		\end{itemize}
		\item[] \file{+ <> createFunctionsDAODynamoDB(db: AWS::DynamoDB): FunctionsDAODynamoDB} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \file{event: LambdaEvent} \\
			Parametro contenente, all'interno del campo \file{body} sotto forma di stringa in formato JSON, un oggetto contenente tutti i dati relativi ad un messaggio da inviare. Tali dati sono:
			\begin{lstlisting}[language=json,firstnumber=1] {
			   "msg":"NotificationMessage",
				 "send\_to":"String"
			  }
				\end{lstlisting} Dove \file{msg} è un oggetto di tipo \file{NotificationMessage}, mentre \file{send\_to} è una stringa contenente il mittente del messaggio;
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dalle lambda function per inviare la risposta. La risposta, contenuta nel \file{LambdaResponse} parametro del metodo \file{LambdaContext::succeed}, possiede un attributo \file{body}, il quale conterrà una stringa vuota. Il risultato delle operazioni di questo metodo sarà deducibile tramite il valore dell'attributo \file{LambaResponse::statusCode};
		\end{itemize}
	\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un NotificationChannel. \\ Per consultare la relativa documentazione, consultare questa pagina \url{https://api.slack.com/types/channel};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ id: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi del topic di un canale Slack. Il Topic di un canale Slack è l'oggetto o tema delle discussioni in esso. Per la relativa documentazione, consultare questa pagina \url{https://api.slack.com/methods/channels.setTopic};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce un meccanismo per impostare e ottenere i valori dei parametri relativi ad un Purpose. Il purpose di un canale Slack è lo scopo per il quale è stato creato. Per la relativa documentazione, consultare la seguente pagina \url{https://api.slack.com/methods/channels.setPurpose};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ value: String} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo contenente il modo con il quale notificare. Questo attributo può assumere uno tra i seguenti valori: \begin{itemize} \item \file{in\_channel}, che mostra il \file{NotificationMessage} ai membri del canale con il message button già cliccato; \item \file{ephemeral}, che mostra il \file{NotificationMessage} ai membri del canale che cliccano sul message button. \end{itemize} Per la relativa documentazione, consultare questa pagina \url{https://api.slack.com/docs/message-buttons};
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi degli Attachment che dovranno essere aggiunti ad un NotificationMessage. Un Attachment, contenuto in un NotificationsMessage, permette di aggiungere del significato ad un NotificationMessage ed arricchirlo tramite immagini, colori ed altro. Per la relativa documentazione, consultare questa pagina \url{https://api.slack.com/docs/message-buttons};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ title: String[0..1]} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di una \file{Action}. La classe \file{Attachment} ne contiene un array. Per la relativa documentazione, consultare la pagina \url{https://api.slack.com/docs/message-buttons};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
\hypertarget{ConfirmationHash_label}{\paragraph{ConfirmationHash}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassConfirmationHash.png}
	\caption{Back-end::Notifications::ConfirmationHash}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{ConfirmationHash};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di rappresentare e organizzare i dati relativi ad un ConfirmationHash;
	\item \textbf{Utilizzo}: fornisce gli attributi di un ConfirmationHash, che dovranno essere aggiunti ad una Action. Un ConfirmationHash;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ title: String[0..1]} \\
...
\hypertarget{NotificationMessageEvent_label}{\paragraph{NotificationMessageEvent}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassNotificationMessageEvent.png}
	\caption{Back-end::Notifications::NotificationMessageEvent}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{NotificationMessageEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe permette alle lambda function l'invio dei dati relativi ad un \file{NotificationMessage}. DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE DA ELIMINARE;
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio alle lambda function di una richiesta contenente i dati relativi ad un \file{NotificationMessage};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ msg: NotificationMessage} \\
		Attributo contenente un NotificationMessage;
		\item[] \file{+ send\_to: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi per rappresentare un messaggio di conferma di Slack. \\ Per la relativa documentazione, consultare la pagina \url{https://api.slack.com/docs/message-buttons};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ title: String[0..1]} \\
...
\begin{itemize}
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createLocalRegistry(): LocalRegistry} \\
...
\begin{figure}[h]
	...
	\caption{Client::Audio::TTS::Player}
\end{figure}
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce al client funzionalità di riproduzione del file audio di risposta, da parte del sistema, relativo a ciò che l'ospite comunica. Permette all'ospite di fermare e riavviare la riproduzione audio e di impostare il volume.\\ È soggetta ad una constructor-based che ha come oggetto un \file{TTSConfig};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- tts: SpeechSynthesis} \\
		Attributo contenente uno SpeechSynthesis di JavaScript Web Speech API il quale fornisce dei metodi per la gestione del file audio. Per la relativa documentazione, consultare questa pagina \url{https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis};
		\item[] \file{- subject: BoolSubject} \\
		Attributo contenente un \file{BoolSubject}, il quale emette item del tipo: \begin{itemize} \item TRUE quando l'utente sta parlando; \item FALSE altrimenti. \end{itemize} Questo attributo serve per disattivare il \file{Recorder} durante la riproduzione del file audio relativo alla risposta, fornita dal sistema;
	...
	\begin{itemize}
		...
		\item[] \file{+ <<Create>> createPlayer(conf: TTSConfig): Player} \\
		Metodo che si occupa di creare un oggetto di tipo Player a partire da un TTSConfig;\\
		Parametri:
		\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::TTS::TTSConfig}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{TTSConfig};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe contiene tutti i parametri relativi alla configurazione di un audio;
	\item \textbf{Utilizzo}: fornisce al \file{Player} un meccanismo per gestire i parametri relativi alla configurazione per la riproduzione di un file audio. Per la relativa documentazione, consultare la pagina \url{https://developer.mozilla.org/it/docs/Web/API/SpeechSynthesisUtterance};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ lang: String} \\
		...
		\item[] \file{+ voice: SpeecSynthesisVoice} \\
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::RecorderWorker}
\end{figure}
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Attributo contenente un valore booleano che indica se il file audio ha uno o due canali. \\ In caso il canale sia uno, allora questo attributo dovrà contenere il valore true;
		\end{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::SpeechEndSubject}
\end{figure}
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce un meccanismo che permette l'invio del file audio contenente il messaggio dell'utente una volta che quest'ultimo ha terminato di parlare. L'observer collegato è LogicObserver;
	\item \textbf{Padre}: \file{Subject};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ next(val: boolean): void} \\
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::RecorderWorkerConfig}
\end{figure}
\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::RecorderMsg}
\end{figure}
\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::RecorderWorkerMsg}
\end{figure}
\begin{itemize}
...
\begin{figure}[h]
	...
	\caption{Client::Audio::Recorder::RecorderConfig}
\end{figure}
\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: ;
	\item \textbf{Figli}: \file{UserObservable}, \file{ConversationObservable}, \file{GuestObservable}, \file{FunctionObservable}, \file{RuleObservable}, \file{AgentObservable}, \file{ErrorObservable};
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi per rappresentare l'oggetto status object relativo ad una richiesta mandata ad api.ai. \\ Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/status-object};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ code: int} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi per rappresentare l'oggetto result relativo ad una risposta di api.ai. \\ Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\hypertarget{WebhookResponseBody_label}{\paragraph{WebhookResponseBody}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassWebhookResponseBody.png}
	\caption{Back-end::Utility::WebhookResponseBody}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{WebhookResponseBody};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di organizzare e rappresentare gli attributi relativi ad una risposta che un webhook deve fornire, come descritto nelle API di api.ai;
	\item \textbf{Utilizzo}: contiene gli attributi della risposta di webhook ad api.ai. Per una descrizione dettagliata si rimanda alla pagina \url{https://docs.api.ai/docs/webhook#section-format-of-response-from-the-service};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ speech: String} \\
		Attributo contenente la risposta testuale alla richiesta;
		\item[] \file{+ displayText: String} \\
		...
		\item[] \file{+ contextOut: ContextArray} \\
		...
		\item[] \file{+ followupEvent: Object} \\
		Attributo contenente parametri opzionali che il servizio web utilizzato vuole inviare ad api.ai;
	\end{itemize}
\end{itemize}

\hypertarget{WebhookRequest_label}{\paragraph{WebhookRequest}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassWebhookRequest.png}
	\caption{Back-end::Utility::WebhookRequest}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{WebhookRequest};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di rappresentare la risposta fornita da api.ai, la quale sarà poi la richiesta da mandare al webhook. DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ??? DA ELIMINARE ???;
	\item \textbf{Utilizzo}: fornisce gli attributi per rappresentare l'oggetto Response relativo ad una richiesta mandata ad api.ai. \\ Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ id: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi necessari a descrivere gli errori che si possono verificare. L'attributo \file{code} contiene un valore uguale a 0 nel caso non si sia verificato nessun errore, diverso da 0 altrimenti. Nel caso di \file{code} diverso da 0, l'attributo \file{msg} contiene una stringa che descrive l'errore verificatosi;
	\item \textbf{Attributi}:
	\begin{itemize}
...
\hypertarget{LambdaEmptyResponse _label}{\paragraph{LambdaEmptyResponse }}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaEmptyResponse .png}
	\caption{Back-end::Utility::LambdaEmptyResponse }
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaEmptyResponse };
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: ??????;
	\item \textbf{Utilizzo}: ???????;
	\item \textbf{Padre}: \file{LambdaVAResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: Object} \\
		???;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaEmptyContext_label}{\paragraph{LambdaEmptyContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaEmptyContext.png}
	\caption{Back-end::Utility::LambdaEmptyContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaEmptyContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: ????;
	\item \textbf{Utilizzo}: ?????;
	\item \textbf{Padre}: \file{LambdaVAContext};
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Figli}: \file{LambdaRuleContext}, \file{LambdaRuleListContext}, \file{LambdaFunctionListContext}, \file{LambdaFunctionContext}, \file{LambdaVAContext};
	\item \textbf{Metodi}:
	\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Figli}: \file{LambdaVAResponse}, \file{LambdaFunctionResponse}, \file{LambdaFunctionListResponse}, \file{LambdaRuleListResponse}, \file{LambdaRuleResponse};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\hypertarget{LambdaWebhookContext_label}{\paragraph{LambdaWebhookContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaWebhookContext.png}
	\caption{Back-end::Utility::LambdaWebhookContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaWebhookContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{Webhook};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutti i \file{Webhook} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ done(res: LambdaWebhookResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaWebhookResponse} \\
			Parametro contenente;
		\end{itemize}
		\item[] \file{+ succeed(res: LambdaWebhookResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaWebhookResponse} \\
		...
		\end{itemize}
		\item[] \file{+ fail(res: LambdaWebhookResponse): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{res: LambdaWebhookResponse} \\
...
\hypertarget{LambdaTokenContext_label}{\paragraph{LambdaTokenContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaTokenContext.png}
	\caption{Back-end::Utility::LambdaTokenContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaTokenContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{Token};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente un \file{Token} presente nel sistema da API Gateway alle lambda function;
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ succeed(token: Token): void} \\
		Metodo che permette di;\\
		Parametri:
		\begin{itemize}
			\item \file{token: Token} \\
			Parametro contenente;
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{LambdaTokenResponse_label}{\paragraph{LambdaTokenResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaTokenResponse.png}
	\caption{Back-end::Utility::LambdaTokenResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaTokenResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{Token};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un \file{Token} come risposta;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: Token} \\
		Attributo contenente;
	\end{itemize}
\end{itemize}

\hypertarget{LambdaStringArrayContext_label}{\paragraph{LambdaStringArrayContext}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaStringArrayContext.png}
	\caption{Back-end::Utility::LambdaStringArrayContext}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaStringArrayContext};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita \file{LambdaContext}, reimplementando il metodo \file{succeed} in modo che accetti parametri di tipo \file{StringArray};
	\item \textbf{Utilizzo}: fornisce un meccanismo per l'invio dell'array contenente tutte le \file{String} presenti nel sistema da API Gateway alle lambda function;
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ succed(stringarray: StringArray): void} \\
		Metodo che permette;\\
		Parametri:
		\begin{itemize}
			\item \file{stringarray: StringArray} \\
			Parametro contenente;
		\end{itemize}
	\end{itemize}
\end{itemize}

\hypertarget{LambdaStringArrayResponse_label}{\paragraph{LambdaStringArrayResponse}}
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/ClassLambdaStringArrayResponse.png}
	\caption{Back-end::Utility::LambdaStringArrayResponse}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaStringArrayResponse};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che eredita da \file{LambdaResponse}, aggiungendo un attributo body di tipo \file{StringArray};
	\item \textbf{Utilizzo}: fornisce alle lambda function un meccanismo per inviare un array di \file{String} come risposta;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ body: StringArray} \\
		Attributo contenente;
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di adattare la classe \file{SpeechToTextV1} all'interfaccia \file{STTModule}. ----------- Questa classe si occupa di convertire l'interfaccia fornita dal servizio di Speech to Text di IBM in una più adatta alle esigenze dell'applicazione, definita da \file{STTModule}. \\ Facendo da Adapter tra le API del servizio di Speech to Text di IBM (adaptee) e l'interfaccia \file{STTModule} (target) utilizzata da \file{APIGateway::VocalAPI}, permette l'interoperabilità tra queste due interfacce;
	\item \textbf{Utilizzo}: implementa l'interfaccia \file{STTModule} utilizzando il servizio di Speech to Text fornito da IBM tramite l'SDK per Node.js fornito da IBM stessa. \\ Per ulteriori informazioni, consultare le documentazioni presenti alle seguenti pagine: \begin{itemize} \item \url{https://www.ibm.com/watson/developercloud/doc/speech-to-text/}; \item \url{https://github.com/watson-developer-cloud/node-sdk#speech-to-text}. \end{itemize} -------------- Fornisce a \file{STTModule} un meccanismo che permette di interrogare le API del servizio Watson Speech to Text di IBM, in modo da consentire a \file{APIGateway::VocalAPI} l'utilizzo di quest'ultime utilizzando un'interfaccia distinta e più consona alle proprie esigenze. \\ È soggetta ad una constructor-based dependency injection, la quale ha come oggetti: \begin{itemize} \item uno \file{StreamBufferModule} contenente ... ??? ; \item uno \file{SpeechToTextV1}. \end{itemize} Per ulteriori informazioni, consultare le documentazioni presenti alle seguenti pagine: \begin{itemize} \item \url{https://www.ibm.com/watson/developercloud/doc/speech-to-text/}; \item \url{https://github.com/watson-developer-cloud/node-sdk#speech-to-text}. \end{itemize};
	\item \textbf{Attributi}:
	\begin{itemize}
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createSTTWatsonAdapter(sb: StreamBufferModule, stt: SpeechToTextV1): STTAdapter} \\
		...
		\end{itemize}
		\item[] \file{+ syncSpeechToText(audio: Buffer, type: String): String} \\
		Implementa il metodo \file{syncSpeechToText} contenuto nell'interfaccia;\\
		Parametri:
		\begin{itemize}
