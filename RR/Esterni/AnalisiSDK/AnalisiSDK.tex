...
\begin{diario}
	...
	\modifica{Mattia Bottaro}{\VER}{Verifica}{2017-01-06}{0.3.0}
	...
	\modifica{Mattia Bottaro}{\VER}{Verifica}{2017-01-04}{0.2.0}
	...
	\modifica{Mattia Bottaro}{\VER}{Verifica}{2017-01-03}{0.1.0}
	\modifica{Luca Bertolini}{\RESP}{Stesura sezioni wit.ai e API.AI}{2017-01-03}{0.0.2}
...
\section{wit.ai}
	\subsection{Descrizione}
		...
		\subsection{Caratteristiche}
			...
			La piattaforma non del tutto stabile (\gl{crash}, \gl{freeze} temporanei, ecc.) e l'assenza di moduli di integrazione diretti con \gl{API} esterne rendono lo sviluppo su wit.ai non ottimale.
...
\begin{itemize}
	...
	\item compatibilità con molti sistemi operativi, tra cui: \gl{iOS}, \gl{Android}, \gl{Windows Phone}, \gl{Raspberry Pi}, \gl{Python} e \gl{C};
...
\section{API.AI}
	...
	\end{minipage}
	\subsection{Caratteristiche}
	API.AI fornisce \gl{SDK} per i principali liguaggi di programmazione tra i quali \gl{C++}, \gl{C\#}, \gl{Java}, \gl{Node.js}, \gl{JavaScript} e \gl{Python}. Inoltre può essere integrato con \gl{Amazon Echo} e \gl{Microsoft Cortana}.\\
...
\newpage
	...
	\subsection{Descrizione}
	\begin{minipage}{0.7\textwidth}\raggedright
		Alexa è stata annunciata per la prima volta nel novembre del 2014 come assistente virtuale di \gl{Amazon Echo}, sviluppato da Amazon.com.
		Le funzionalità di Alexa sono ispirate dal computer di bordo delle navicelle interstellari presenti nelle serie TV sci-fi \quotes{Star Trek}.
	...
	\end{minipage}
	\subsection{Caratteristiche}
		Amazon.com mette a disposizione gratuitamente Alexa Voice Service (AVS), un servizio che permette di integrare, in qualsiasi dispositivo e applicazione con accesso al web, l'assistente virtuale Alexa. La compagnia inoltre fornisce l'Alexa Skills Kit (ASK), un \gl{SDK} che permette di estendere le funzionalità dell'assistente virtuale con delle Custom Skill. Per creare una Custom Skill ASK richiede:
		\begin{itemize}
			...
			\item \textbf{Servizio \textit{\gl{cloud}}}: accetta gli intenti come richieste strutturate ed agisce in base ad essi;
		...
		\end{itemize}
	Amazon.com suggerisce di usare \gl{AWS Lambda} per la creazione di servizi \textit{\gl{cloud}}.
...
\newpage
	...
	\begin{table}[h]
		...
		\hline 
		\rule[-0.2cm]{0cm}{0.6cm}Creazione Intent con NLU & \cmark & \cmark & \cmark & \cmark \\
		\hline 
		\rule[-0.2cm]{0cm}{0.6cm}Intents personalizzabili & \cmark & \cmark & \xmark & \cmark \\
		\hline 
		...
		\hline 
		Costi & gratis & gratis senza ambiti predefiniti & gratis & \rule[-0.2cm]{0cm}{0.6cm} gratis fino a 1.000.000 di chiamate (per \gl{AWS Lambda})\\
		\hline 
		Piattaforme e linguaggi  di prog. supportati & \textit{\gl{Node.js}; \gl{Python}; \gl{Ruby}; \gl{HTTP}} & \rule[-0.2cm]{0cm}{0.6cm} \textit{\gl{Android}; \gl{iOS}; \gl{Apple Watch}; \gl{Node.js}; \gl{Cordova}; \gl{Unity}; \gl{C\#}; \gl{Xamarin}; \gl{Windows Phone}; \gl{Python}; \gl{JavaScript}; \gl{PHP}; \gl{Botkit}; \gl{C++}; \gl{Mac OS X}; \gl{HTML5}; \gl{Ruby}} & \textit{\gl{iOS}} & qualsiasi linguaggio permetta di accettare richieste \textit{\gl{HTTPS}}\\
		\hline 
		\rule[-0.2cm]{0cm}{0.6cm}Documentazione \gl{API} esaustiva & \cmark & \cmark & \cmark & \cmark	\\	
		\hline
		\rule[-0.2cm]{0cm}{0.6cm}Gui modificazione Intents & \cmark & \cmark & \xmark & \cmark \\
		\hline 
		Autenticazione & \gl{OAuth2} & \gl{double token} & \xmark & \rule[-0.2cm]{0cm}{0.6cm}LWA(\textit{Login with Amazon}) \\
		\hline
...
\newpage
	\section{Conclusioni}
	Dall'attenta analisi del gruppo \GRUPPO{} è emerso che SiriKit non è adeguato a soddisfare le esigenze del \gl{progetto} \PROGETTO{} in quanto gli ambiti forniti sono limitati, le interazioni con l'utente non sono personalizzabili ed il supporto a piattaforme diverse da \gl{iOS} è assente.\\
	I restanti \gl{SDK} non presentano tali limitazioni e quinidi potrebbero essere tutti potenzialmente sfruttati. La scelta del gruppo è ricaduta su API.AI per la maggiore stabilità rispetto a wit.ai, per il  maggior numero di lingue disponibili in vista di un supporto futuro ad interazioni con utenti di diversa nazionalità, per la maggiore maturità della piattaforma e per la possibilità di esportare gli Agents in formati compatibili con i principali assistenti virtuali presenti sul mercato, tra cui Cortana ed Alexa.
