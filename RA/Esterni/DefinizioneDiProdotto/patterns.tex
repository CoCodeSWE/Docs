  ...
  \subsection{Strutturali}
    \subsubsection{Façade}
      \begin{itemize}
       ...
       Nel contesto della progettazione architetturale del progetto \PROGETTO, questo pattern non è stato usato nella sua forma pura (visibile in figura \ref{fig:facade}). Facendo però un utilizzo di un \gl{API Gateway}, il quale nasconde la complessità del back-end al client fornendone anche i meccanismi di utilizzo, si è fatto utilizzo dei concetti che questo pattern rispetta (figura \ref{fig:facade_atavi});
	\item \textbf{Vantaggi:}
	  \begin{itemize}
