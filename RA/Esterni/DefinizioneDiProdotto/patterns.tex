...
\end{figure}
    ...
    \subsubsection{Dependency Injection}
     \begin{itemize}
       \item \textbf{Scopo:} la \gl{dependency injection} consiste nella separazione del comportamento di una componente dalla risoluzione delle sue dipendenze. La \textit{constructor injection} avviene quando le dipendenze vengono dichiarate nel costruttore (figura \ref{fig:constr_dep}), mentre la \textit{setter injection} quando esse vengono dichiarate come metodi setter (figura \ref{fig:setter_dep}).
	\item \textbf{Vantaggi:}
	  \begin{itemize}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.5\textwidth,height=\textheight,keepaspectratio]{images/constructor_dep.png}
		\caption{Constructor injection}\label{fig:constr_dep}
	\end{figure}
	\begin{figure}[h]
		\centering
		\includegraphics[width=\textwidth,height=\textheight,keepaspectratio,scale=0.1]{images/setter_dep.png}
		\caption{Setter injection}\label{fig:setter_dep}
	\end{figure}
\newpage
  \subsection{Strutturali}
    \subsubsection{Façade}
      \begin{itemize}
       ...
       Nel contesto della progettazione architetturale del progetto \PROGETTO, questo pattern non è stato usato nella sua forma pura (visibile in figura \ref{fig:facade}). Facendo però un utilizzo di un \gl{API \gl{Gateway}}, il quale nasconde la complessità del back-end al client fornendone anche i meccanismi di utilizzo, si è fatto utilizzo dei concetti che questo pattern rispetta (figura \ref{fig:facade_atavi});
	\item \textbf{Vantaggi:}
	  \begin{itemize}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=\textwidth,height=\textheight,keepaspectratio,scale=0.1]{images/facadepattern.png}
		\caption{Pattern Façade nella nostra architettura}\label{fig:facade_atavi}
	\end{figure}
	\newpage
...
È utilizzato da diverse classi del sistema per osservare e gestire gli eventi generati dalle classi Subject, ma sempre secondo lo schema in figura \ref{fig:obs2}.
	\begin{figure}[h]
		\centering
