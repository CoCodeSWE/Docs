...
\gl{Package} contenente tutte le componenti che costituiscono il back-end. Le componenti sono organizzate secondo il pattern a \gl{microservizi}.   \begin{figure}[h] \centering \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/diagrams/back-end/Official_Backend_0304/Back-end.png}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce il metodo che si occupa di rispondere alle chiamate al microservizio da parte dell'\file{Agent} di amministrazione;
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ webhook(event: LambdaEvent, context: LambdaContext): void} \\		Questo metodo soddisfa i requisiti per webhook descritti da api.ai. Viene chiamato ad ogni interazione dell'agente di amministrazione, e si occupa di verificare che nella richiesta sia presente un JSON Web Token (JWT) che confermi un'autenticazione avvenuta con successo. In caso di mancata autenticazione, il campo \file{data.status} della risposta sarà impostato a 403.  Nel caso in cui il token sia presente e valido (la firma sia valida ed il token non sia scaduto), tale campo sarà invece impostato a 200, ed il campo \file{speech} sarà copiato da \file{fulfillment.speech} della richiesta, risultando quindi in una risposta uguale a quella definita nell'agente di api.ai;\\
		Parametri:
		\begin{itemize}
...
\end{lstlisting}
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response} (visitato in data 2017-04-22);
...
\begin{itemize}
    ...
    \item \file{name} indica nome e cognome dell'amministratore;
...
\begin{itemize}
    ...
    \item \file{membe\_name} indica il nome e il cognome della persona desiderata;
\end{itemize};
		\end{itemize}
		\item[] \file{+ <<Create>> createConversationWebhookService(conversations: ConversationsDAO\\, guests: GuestsDAO, users: UsersDAO): ConversationWebhookService} \\		Metodo che permette la costruzione di un \file{ConversationWebhookService}. Permette la dependency injection che ha come oggetti un \file{ConversationsDAO}, \file{GuestsDAO} e \file{UsersDAO};\\
		Parametri:
		\begin{itemize}
			\item \file{conversations: ConversationsDAO} \\
...
\begin{itemize}
 \item le funzionalità da API Gateway, permettendo l'interazione \file{Client} - \file{Back-end} tramite la pubblicazione di appositi \gl{endpoints};    \item la mappatura delle richieste del \file{Client} nelle funzionalità supportate dal \file{Back-end};
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo contenente il \file{RequestPromiseModule} di cui è stata eseguita la dependency injection nel costruttore. Viene utilizzato per effettuare richieste HTTP sostituendo \gl{callback} con promises;
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{- addUserEnrollment(enr: Enrollment): ErrorObservable} \\		Metodo che permette di aggiungere un enrollment ad un utente del sistema. Restituisce un oggetto di tipo \file{ErrorObservable} e, in caso di errore, notifica l'\file{ErrorObserver} chiamando il suo metodo \file{error} utilizzando come parametro un oggetto \file{Exception} con code impostato a 1 nel caso in cui l'utente non esista. In assenza di errori, viene chiamato il metodo il metodo \file{complete};\\
		Parametri:
		\begin{itemize}
		...
		\end{itemize}
		\item[] \file{- getUserList(query: UserListQueryParameters): UserArrayObservable}\\ \\		Metodo che permette di ottenere una lista degli utenti del sistema;\\
		Parametri:
		\begin{itemize}
			\item \file{query: UserListQueryParameters} \\
		...
		\end{itemize}
		\item[] \file{- addRule(rule: Rule): ErrorObservable} \\		Metodo che permette di aggiungere una direttiva al sistema. Restituisce un oggetto di tipo \file{ErrorObservable};\\
		Parametri:
		\begin{itemize}
			\item \file{rule: Rule} \\
		...
		\end{itemize}
		\item[] \file{- getRuleList(): RuleArrayObservable} \\		Metodo che permette di ottenere la lista delle direttive del sistema.
;\\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Parametro contenente l'id della sessione della conversazione da ricevere;
		\end{itemize}
		\item[] \file{+ getConversationList(query: ConversationListQueryParameters): \\ ConversationObservable}\\ \\		Implementazione del metodo dell'interfaccia \file{ConversationsDAO}. Utilizza il metodo \file{scan} del \file{DocumentClient} per ottenere la lista delle conversazioni dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{query: ConversationListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione;
...
\FloatBarrier
\newpage
\subsubsection{Back-end::Curiosity}
Package contenente le classi e le interfacce che realizzano il microservizio relativo alla gestione delle curiosità da comunicare all'ospite. \\
Vengono offerte funzionalità che permettono:
\begin{itemize}
\item la lettura di una singola curiosità, presa a caso tra quelle non ancora comunicate all'ospite nell'interazione corrente, in base al tipo richiesto. 
\end{itemize}

Sono contenute le classi che realizzano:
\begin{itemize} 
 \item la definizione dei dati relativi ad una curiosità; 
 \item il microservizio che espone gli endpoint per permetterne il suo utilizzo. 
 \item l'interfaccia per l'interazione con il database contenente le curiosità;
 \item le modalità con le quali i dati devono essere trasmessi da e al database;
 \item il microservizio che espone gli endpoint per permetterne il suo utilizzo.
\end{itemize}
\begin{figure}[h] \centering \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/diagrams/back-end/Official_Backend_0304/Curiosity.png}
	\caption{Package Back-end::Curiosity}
\end{figure}
\paragraph{Classi}
\hypertarget{ CuriosityDAO_label}{\subparagraph{ CuriosityDAO}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosityDAO.png}
	\caption{Back-end::Curiosity:: CuriosityDAO}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{CuriosityDAO};
	\item \textbf{Tipo}: \file{Interface};
	\item \textbf{Descrizione}: questa classe si occupa di astrarre le modalità d'interazione con il database contenente le curiosità;
	\item \textbf{Utilizzo}: fornisce a \file{CuriosityWebhookService} un meccanismo per accedere al database contenente le curiosità, senza conoscerne le modalità di implementazioni e di persistenza di quest'ultimo. Permette operazioni di lettura, scrittura e rimozione di curiosità;
	\item \textbf{Figlio}: \file{CuriosityDAODynamoDB};
	\item \textbf{Metodi}: 
	\begin{itemize}
		\item[] \file{+ getCuriosity(type: String): CuriosityObservable} \\		Metodo che permette di ottenere una curiosità, la quale sarà casuale e non ancora comunicata all'ospite nell'interazione corrente, in base al tipo di curiosità \file{type}.
 L'\file{Observable} restituito riceverà l'oggetto rappresentante tale \file{Curiosity}, e verrà completato. Nel caso in cui non sia possibile ottenere una curiosità, gli \file{Observer} interessati non riceveranno alcun valore, ma verranno notificati tramite la chiamata del loro metodo \file{error}.\\
Questo metodo ha tra i parametri quattro array, ognuno dei quali contenenti le curiosità di un determinato tipo. Quando viene richiesta una curiosità di un certo tipo, essa viene estratta dal relativo array, per poi essere restituita al chiamante;\\
		Parametri: 
		\begin{itemize}
			\item \file{type: String} \\
			Parametro contente il tipo di curiosità richiesto dall'ospite.\\
...
\end{itemize};
	...
	\item \textbf{Relazioni con le altre classi}: 
	\begin{itemize}
		\item IN \hyperlink{CuriosityWebhookService_label}{\file{CuriosityWebhookService}}
		\item IN \hyperlink{CuriosityService_label}{\file{CuriosityService}}
		\item OUT \hyperlink{CuriosityObservable_label}{\file{CuriosityObservable}}
		\item OUT \hyperlink{ErrorObservable_label}{\file{ErrorObservable}}
		\item OUT \hyperlink{Curiosity_label}{\file{Curiosity}}
	\end{itemize}
\end{itemize}
\FloatBarrier

\hypertarget{Curiosity_label}{\subparagraph{Curiosity}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosity.png}
	\caption{Back-end::Curiosity::Curiosity}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{Curiosity};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di raggruppare le informazioni relative ad una curiosità;
	\item \textbf{Utilizzo}: fornisce l'accesso ai dati relativi ad una curiosità;
	\item \textbf{Attributi}: 
	\begin{itemize}
		\item[] \file{+ id: String} \\
		Stringa contenente l'id della curiosità;
		\item[] \file{+ type: String} \\
		Stringa contenente il tipo della curiosità;
		\item[] \file{+ text: String} \\
		Stringa contenente il testo della curiosità;
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}: 
	\begin{itemize}
		\item IN \hyperlink{<<interface>> CuriosityDAO_label}{\file{<<interface>> CuriosityDAO}}
		\item IN \hyperlink{CuriosityObserver_label}{\file{CuriosityObserver}}
		\item IN \hyperlink{CuriosityDAODynamoDB_label}{\file{CuriosityDAODynamoDB}}
	\end{itemize}
\end{itemize}
\FloatBarrier

\hypertarget{CuriosityDAODynamoDB_label}{\subparagraph{CuriosityDAODynamoDB}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosityDAODynamoDB.png}
	\caption{Back-end::Curiosity::CuriosityDAODynamoDB}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{CuriosityDAODynamoDB};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che si occupa di implementare l'interfaccia \file{CuriosityDAO}, utilizzando un database DynamoDB come supporto per la memorizzazione dei dati;
	\item \textbf{Utilizzo}: implementa i metodi dell'interfaccia \file{CuriosityDAO} interrogando un database DynamoDB. Utilizza \file{AWS::DynamoDB::DocumentClient} per l'accesso al database. La dependency injection dell'oggetto \file{AWS::DynamoDB} viene fatta utilizzando il costruttore;
	\item \textbf{Padre}: \file{<<interface>> CuriosityDAO};
	\item \textbf{Attributi}: 
	\begin{itemize}
		\item[] \file{- client: AWS::DynamoDB::DocumentClient} \\
		Attributo contenente un riferimento al modulo di Node.js utilizzato per l'accesso al database DynamoDB contenente la tabella delle curiosità;
	\end{itemize}
	\item \textbf{Metodi}: 
	\begin{itemize}
		\item[] \file{+ getCuriosity(type: String): CuriosityObservable} \\		Implementazione del metodo definito nell'interfaccia \file{CuriosityDAO}. Utilizza il metodo \file{get} del \file{DocumentClient} per ottenere i dati relativi ad una \file{Curiosity} dal database;\\
		Parametri: 
		\begin{itemize}
			\item \file{type: String} \\
			Parametro contente il tipo di curiosità richiesto dall'ospite.\\ Può assumere uno tra i seguenti valori: \begin{itemize} \item "food"; \item "general"; \item "sport"; \item "technology". \end{itemize};
		\end{itemize}
		\item[] \file{+ <<Create>> createRulesDAODynamoDB(client: AWS::DynamoDB::DocumentClient)\\: CuriosityDAODynamoDB} \\		Constructor della classe \file{CuriosityDAODynamoDB}. Permette di effettuare la dependency injection di \file{AWS::DynamoDB};\\
		Parametri: 
		\begin{itemize}
			\item \file{client: AWS::DynamoDB::DocumentClient} \\
...
\hypertarget{CuriosityObservable_label}{\subparagraph{CuriosityObservable}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosityObservable.png}
	\caption{Back-end::Curiosity::CuriosityObservable}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{CuriosityObservable};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe implementa un \file{Observable} che permette l'iscrizione di \\ \file{CuriosityObserver};
	\item \textbf{Utilizzo}: fornisce i meccanismi necessari per il passaggio di una serie di \file{Curiosity} ad un \file{Observer} interessato;
	\item \textbf{Padre}: \file{Observable};
	\item \textbf{Metodi}: 
	\begin{itemize}
		\item[] \file{+ <>createCuriosityObservable(onSubscription: function(observer: \\CuriosityObserver) : void): CuriosityObservable} \\		Constructor di \file{CuriosityObservable};\\
		Parametri: 
		\begin{itemize}
			\item \file{onSubscription: function(observer: CuriosityObserver) : void} \\
			Funzione che verrà eseguita quando un \file{Observer} si iscrive all'\file{Observable}. Si occupa di passare i dati all'\file{Observer}, chiamando il metodo \file{next(curiosity: Curiosity)}. Quando non ci sono più dati da restituire, si occupa di chiamare il metodo \file{complete()}. Nel caso in cui si verificasse un errore, si occupa di chiamare il metodo \file{error(err: Object)} con i dati relativi all'errore verificatosi;
		\end{itemize}
		\item[] \file{+ subscribe(observer: CuriosityObserver): Subscription} \\		Metodo che permette ad uno \file{CuriosityObserver} interessato di iscriversi a questo \file{Observable};\\
		Parametri: 
		\begin{itemize}
			\item \file{observer: CuriosityObserver} \\
			\file{Observer} che si vuole iscrivere;
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}: 
	\begin{itemize}
		\item IN \hyperlink{<<interface>> CuriosityDAO_label}{\file{<<interface>> CuriosityDAO}}
		\item IN \hyperlink{CuriosityDAODynamoDB_label}{\file{CuriosityDAODynamoDB}}
		\item OUT \hyperlink{CuriosityObserver_label}{\file{CuriosityObserver}}
	\end{itemize}
\end{itemize}
\FloatBarrier

\hypertarget{CuriosityObserver_label}{\subparagraph{CuriosityObserver}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosityObserver.png}
	\caption{Back-end::Curiosity::CuriosityObserver}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{CuriosityObserver};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che rappresenta un \file{Observer} che si aspetta dati di tipo \file{Curiosity};
	\item \textbf{Utilizzo}: ridefinisce il metodo \file{next()} della classe \file{ObserverAdapter}, in maniera tale che accetti dati di tipo \file{Curiosity};
	\item \textbf{Metodi}: 
	\begin{itemize}
		\item[] \file{+ next(curiosity: Curiosity): void} \\		 Metodo che permette agli \file{Observable} di notificare l'\file{Observer} con dati di tipo \file{Curiosity}. Definisce inoltre le operazioni che l'\file{Observer} compierà all'arrivo di tali dati;\\
		Parametri: 
		\begin{itemize}
			\item \file{curiosity: Curiosity} \\
			Parametro contenente la \file{Curiosity} mandata dall'\file{Observable};
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}: 
	\begin{itemize}
		\item IN \hyperlink{CuriosityObservable_label}{\file{CuriosityObservable}}
		\item OUT \hyperlink{Curiosity_label}{\file{Curiosity}}
	\end{itemize}
\end{itemize}
\FloatBarrier

\hypertarget{CuriosityService_label}{\subparagraph{CuriosityService}}
\begin{figure}[h]
	\centering
	\includegraphics[scale=1,width=\textwidth,height=\textheight,keepaspectratio]{images/ClassCuriosityService.png}
	\caption{Back-end::Curiosity::CuriosityService}
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{CuriosityService};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe si occupa di realizzare il microservizio \file{Curiosity};
	\item \textbf{Utilizzo}: fornisce i metodi che implementano le lambda function necessarie per ottenere le curiosità presenti nel database;
	\item \textbf{Attributi}: 
	\begin{itemize}
		\item[] \file{- curiosity: CuriosityDAO} \\
		Attributo che permette di contattare \file{CuriosityDAO}, il quale fornisce i meccanismi d'accesso al database delle curiosità;
	\end{itemize}
	\item \textbf{Metodi}: 
	\begin{itemize}
		\item[] \file{+ getCuriosity(event: LambdaIdEvent, context: LambdaContext): void}\\ \\		Metodo che implementa la lambda function che si occupa di recuperare una curiosità, casuale e non ancora comunicata all'ospite, di un determinato tipo;\\
		Parametri: 
		\begin{itemize}
			\item \file{event: LambdaIdEvent} \\
			Parametro contenente, all'interno del campo pathParameters, il tipo dal quale scegliere una curiosità;
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dalle lambda function per inviare la risposta. Il \file{body} del \file{LambdaResponse}, ottenuto dal metodo \file{LambdaContext::succeed}, conterrà, sotto forma di stringa in formato JSON, un oggetto di tipo \file{Curiosity}, contenente i dati relativi alla \file{Curiosity} ritornata;
		\end{itemize}
		\item[] \file{+ <<Create>> createCuriosityService(curiosity: CuriosityDAO): CuriosityService}\\ \\		Metodo che permette di creare un \file{CuriosityService}. Permette la dependency injection avente come oggetto un \file{CuriosityDAO};\\
		Parametri: 
		\begin{itemize}
			\item \file{curiosity: CuriosityDAO} \\
			Attributo contenente il \file{CuriosityDAO};
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}: 
	\begin{itemize}
		\item OUT \hyperlink{<<interface>> CuriosityDAO_label}{\file{<<interface>> CuriosityDAO}}
		\item OUT \hyperlink{LambdaContext_label}{\file{LambdaContext}}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Parametro contenente l'azienda di provenienza dell'ospite;
		\end{itemize}
		\item[] \file{+ getGuestList(query: GuestListQueryParameters): GuestObservable}\\ \\		Implementazione del metodo dell'interfaccia \file{GuestsDAO}. Utilizza il metodo \file{scan} del \file{DocumentClient} per ottenere la lista degli ospiti dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{query: GuestListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione;
...
\hypertarget{MemberListQueryParameters_label}{\subparagraph{MemberListQueryParameters}}
\begin{figure}[h]
	\centering
...
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{MemberListQueryParameters};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/members} del microservizio Members;
	\item \textbf{Utilizzo}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/members} del microservizio Members;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{- name: String} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getMemberList(query: MemberListQueryParameters): MemberObservable}\\ \\		Implementazione del metodo del'interfaccia. Utilizza il metodo \file{users.list} per ottenere le informazioni necessarie da Slack;\\
		Parametri:
		\begin{itemize}
			\item \file{query: MemberListQueryParameters} \\
...
Per la relativa documentazione, consultare la pagina \url{https://api.slack.com/docs/message-buttons}  (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
		...
		Attributo che definisce lo stile del bottone. Per una lista degli stili disponibili e una loro descrizione fare riferimento alla documentazione di Slack (\url{https://api.slack.com/docs/message-buttons#action_fields} (visitato in data 2017-04-22));
		\item[] \file{+ type: String} \\
		Attributo contenente il tipo dell'azione. Al momento l'unico valore accettato è "button". Fare riferimento alle API di   Slack per informazioni aggiornate (\url{https://api.slack.com/docs/message-buttons#action_fields} (visitato in data 2017-04-22));
...
Per la relativa documentazione, consultare questa pagina \url{https://api.slack.com/docs/message-buttons}  (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
...
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{ChannelListQueryParameters};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/notifications} del microservizio Notifications;
	\item \textbf{Utilizzo}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/notifications} del microservizio Notifications;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
\hypertarget{LambdaChannelListEvent_label}{\subparagraph{LambdaChannelListEvent}}
\begin{figure}[h]
	\centering
...
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaChannelListEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe rappresenta l'\file{event} della Lambda Function implementata da \file{NotificationsService.getChannelList};
	\item \textbf{Utilizzo}: viene utilizzata dal metodo \file{getChannelList} di \file{NotificationsService}. Rappresenta la richiesta ricevuta dall'endpoint del microservizio Notifications;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ queryStringParameters: ChannelListQueryParameters} \\
...
Per consultare la relativa documentazione, consultare questa pagina \url{https://api.slack.com/types/channel} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ id: String} \\
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: fornisce gli attributi di un \file{NotificationChannel}. \\ Per consultare la relativa documentazione, consultare questa pagina \url{https://api.slack.com/docs/message-formatting} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ text: String} \\
...
\item la definizione dei dati di un \gl{task};
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getRuleList(query: RuleListQueryParameters): RuleObservable} \\\\		L'\file{Observable} restituito manderà agli \file{Observer} le direttive ottenute, una alla volta, e poi chiama il loro metodo \file{complete}. Nel caso in cui si verifichi un errore, gli \file{Observer} iscritti verranno notificati tramite la chiamate del loro metodo \file{error} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{query: RuleListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione.
;
		\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ getTaskList(query: TaskListQueryParameters): TaskObservable} \\\\		Metodo che permette di ottenere la lista dei compiti. L'\file{Observable} restituito manderà agli \file{Observer} i compiti ottenuti, una alla volta, e poi chiama il loro metodo \file{complete}. Nel caso in cui si verifichi un errore, gli \file{Observer} iscritti verranno notificati tramite la chiamate del loro metodo \file{error} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{query: TaskListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione;
		\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Parametro contenente l'id della \file{Rule} da recuperare;
		\end{itemize}
		\item[] \file{+ getRuleList(query: RuleListQueryParameters): RuleObservable} \\\\		Implementazione del metodo dell'interfaccia \file{RulesDAO}. Utilizza il metodo scan del \\ \file{DocumentClient} per ottenere la lista delle Rule dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{query: RuleListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione;
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getRuleList(event: LambdaRuleListEvent, context: LambdaContext)\\: void} \\		Metodo che implementa la Lambda Function che si occupa di ottenere l'array delle \file{Rule};\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaRuleListEvent} \\
		...
		\end{itemize}
		\item[] \file{+ getTaskList(event: LambdaTaskListEvent, context: LambdaContext)\\: void} \\		Metodo che implementa la Lambda Function che si occupa ottenere l'array delle \file{Task} disponibili;\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaTaskListEvent} \\
			Parametro contenente, all'interno del campo body, una stringa vuota;
			\item \file{context: LambdaContext} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getTask(type: String): TaskObservable} \\		Implementazione del metodo definito nell'interfaccia \file{TasksDAO}. Utilizza il metodo get del \file{DocumentClient} per ottenere i dati relativi ad una \file{Task} dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{type: String} \\
			Parametro contenente il tipo identificativo della funzione;
		\end{itemize}
		\item[] \file{+ getTaskList(query: TaskListQueryParameters): TaskObservable} \\\\		Implementazione del metodo dell'interfaccia \file{TasksDAO}. Utilizza il metodo scan del \\ \file{DocumentClient} per ottenere la lista dei \file{Task} dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{query: TaskListQueryParameters} \\
			Parametro che contiene i valori che verranno passati al FilterExpression dell'interrogazione;
		\end{itemize}
		\item[] \file{+ removeTask(type: String): ErrorObservable} \\		Implementazione del metodo dell'interfaccia \file{TasksDAO}. Utilizza il metodo delete del \file{DocumentClient} per eliminare un \file{Task} dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{type: String} \\
			Parametro contenente il tipo identificativo della funzione;
		\end{itemize}
		\item[] \file{+ updateTask(task: Task): ErrorObservable} \\		Implementazione del metodo dell'interfaccia \file{TasksDAO}. Utilizza il metodo update del \file{DocumentClient} per aggiornare i dati relativi ad un \file{Task} presente all'interno del database;\\
		Parametri:
		\begin{itemize}
			\item \file{task: Task} \\
...
\begin{itemize}
    \item \url{https://www.ibm.com/watson/developercloud/doc/speech-to-text/}  (visitato in data 2017-04-22);
    \item \url{https://github.com/watson-developer-cloud/node-sdk#speech-to-text}  (visitato in data 2017-04-22).
\end{itemize};
...
L'\file{Observable} restituito non riceverà alcun valore, ma verrà completato in caso di aggiunta dell'utente avvenuta con successo. In caso di errore durante l'aggiunta dell'utente, gli \file{Observer} interessati verranno notificati tramite la chiamata del loro metodo \file{error()} con i dati relativi all'errore verificatosi;\\
		...
		\end{itemize}
		\item[] \file{+ getUser(username: String): UserObservable} \\		Metodo che permette di ottenere i dati relativi ad un utente a partire dal suo \file{username}. \\
L'\file{Observable} restituito riceverà l'oggetto rappresentante tale \file{User}, e verrà completato. Nel caso in cui l'utente richiesto non sia presente nel database, gli \file{Observer} interessati non riceveranno alcun valore, ma verranno notificati tramite la chiamata del loro metodo \file{error()};\\
		...
		\end{itemize}
		\item[] \file{+ getUserList(query: UserListQueryParameters): UserObservable} \\\\		L'\file{Observable} restituito manderà agli \file{Observer} gli utenti ottenuti, uno alla volta, e poi chiama il loro metodo \file{complete}. Nel caso in cui si verifichi un errore, gli \file{Observer} iscritti verranno notificati tramite la chiamate del loro metodo \file{error} con i dati relativi all'errore verificatosi;\\
		Parametri:
		\begin{itemize}
			\item \file{query: UserListQueryParameters} \\
			Parametro che indica quali utenti devono essere inclusi nella lista restituita;
		...
		\end{itemize}
		\item[] \file{+ getUsername(name: String): UserObservable} \\		Metodo che permette di ottenere i dati relativi ad un utente a partire dal suo \file{name}. \\ L'\file{Observable} restituito riceverà l'oggetto rappresentante tale \file{User}, e verrà completato. Nel caso in cui l'utente richiesto non sia presente nel database, gli \file{Observer} interessati non riceveranno alcun valore, ma verranno notificati tramite la chiamata del loro metodo \file{error()};\\
		Parametri:
		\begin{itemize}
			\item \file{name: String} \\
			Parametro contenente il \file{name} dello \file{User} che si vuole ottenere;
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
...
\hypertarget{LambdaUserListEvent_label}{\subparagraph{LambdaUserListEvent}}
\begin{figure}[h]
	\centering
...
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{LambdaUserListEvent};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: questa classe rappresenta l'\file{event} della Lambda Function implementata da \file{UsersService.getUserList};
	\item \textbf{Utilizzo}: viene utilizzata dal metodo \file{getUserList} di \file{UsersService}. Rappresenta la richiesta ricevuta dall'endpoint del microservizio Users;
	\item \textbf{Padre}: \file{LambdaEvent};
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ queryStringParameters: UserListQueryParameters} \\
		Oggetto che rappresenta i parametri query della richiesta ricevuta dal microservizio;
	\end{itemize}
\end{itemize}
\FloatBarrier
...
Per la relativa documentazione consultare la pagina \url{https://www.microsoft.com/cognitive-services/en-us/speaker-recognition-api/documentation}  (visitato in data 2017-04-22).
;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ id: String} \\
...
\hypertarget{UserListQueryParameters_label}{\subparagraph{UserListQueryParameters}}
\begin{figure}[h]
	\centering
...
\end{figure}
\begin{itemize}
	\item \textbf{Nome}: \file{UserListQueryParameters};
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/users} del microservizio Users;
	\item \textbf{Utilizzo}: oggetto che rappresenta i parametri che permettono di ridurre i risultati ricevuta da una richiesta all'endpoint \file{/users} del microservizio Users;
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
		Attributo contenente il nome e cognome dell'utente;
		\item[] \file{+ slack\_channel: String} \\
		Attributo contenente il canale Slack dell'utente registrato;
	\end{itemize}
\end{itemize}
\FloatBarrier
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ getUser(username: String): UserObservable} \\		Implementazione del metodo definito nell'interfaccia \file{UsersDAO}. Utilizza il metodo \file{get} del \file{DocumentClient} per ottenere i dati relativi ad uno \file{User} dal database a partire dal suo \file{username};\\
		Parametri:
		\begin{itemize}
			\item \file{username: String} \\
			Parametro contenente lo \file{username} dello \file{User} che si vuole ottenere;
		\end{itemize}
		\item[] \file{+ getUserList(query: UserListQueryParameters): UserObservable} \\\\		Implementazione del metodo dell'interfaccia \file{UsersDAO}. Utilizza il metodo \file{scan} del \file{DocumentClient} per ottenere la lista degli utenti dal database;\\
		Parametri:
		\begin{itemize}
			\item \file{query: UserListQueryParameters} \\
			Parametro che indica quali utenti devono essere inclusi nella lista restituita;
...
Questa classe non interagisce direttamente con il database, ma fa utilizzo di \file{UsersDAO}, il quale nasconde i meccanismi di accesso e persistenza dei dati nel database;
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ getUserList(event: LambdaUserListEvent, context: LambdaContext)\\: void} \\		Metodo che implementa la Lambda Function che si occupa di restituire l'array degli utenti registrati;\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaUserListEvent} \\
			Parametro che rappresenta la richiesta ricevuta dal \file{VocalAPI}. Il campo \file{body} di questo attributo conterrà una stringa vuota;
			\item \file{context: LambdaContext} \\
		...
		\begin{itemize}
			...
			\item \file{context: LambdaContext} \\
			Parametro utilizzato dalle Lambda Function per inviare la risposta. Il \file{body} del \file{LambdaResponse}, parametro del metodo \file{LambdaContext::succeed}, conterrà una stringa vuota e il risultato di questa operazione sarà deducibile dal valore dell'attributo \file{LambdaResponse::statusCode};
		\end{itemize}
...
È soggetta ad una constructor-based dependency injection, la quale ha come oggetto un \file{RequestPromiseModule};
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createVocalLoginMicrosoftModule(conf: VocalLoginModuleConfig\\, rp: RequestPromiseModule): VocalLoginMicrosoftModule} \\		Metodo che permette di costruire un \file{VocalLoginMicrosoftModule}. Permette la dependency injection che ha come oggetto un \file{VocalLoginModuleConfig};\\
		Parametri:
		\begin{itemize}
...
\begin{itemize}
	...
	\item \textbf{Figli}: \file{LambdaIdEvent}, \file{LambdaUserListEvent};
	\item \textbf{Attributi}:
	\begin{itemize}
...
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/status-object} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ code: int} \\
...
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/webhook} (visitato in data 2017-04-22);
	\item \textbf{Figli}: \file{ConversationWebhookService}, \file{AdministrationWebhookService};
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ webhook(event: LambdaEvent, context: LambdaContext): void} \\		Metodo che fornisce l'interfaccia di una Lambda Function compatibile con i requisiti per webhook di api.ai;\\
		Parametri:
		\begin{itemize}
			\item \file{event: LambdaEvent} \\
...
Per la relativa documentazione, consultare la pagina Per una descrizione dettagliata si rimanda alla pagina \\ \url{https://docs.api.ai/docs/webhook#section-format-of-response-from-the-service}  (visitato in data 2017-04-22).
;
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}:
...
Per la relativa documentazione, consultare questa pagina \url{https://docs.api.ai/docs/concept-agents} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
È soggetta ad una constructor-based dependency injection, la quale ha come oggetto un \file{Agent} relativo alle richieste da inviare;
	...
	\begin{itemize}
		...
		Attributo contenente il campo \file{versioning} da inviare in una richiesta ad api.ai. Per chiarire quanto appena detto, consultare la pagina \url{https://docs.api.ai/docs/versioning}  (visitato in data 2017-04-22);
		\item[] \file{- request\_promise: RequestPromiseModule} \\
		Attributo contenente un riferimento al modulo \file{RequestPromise} di Node.js;
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ <<Create>> createApiAiVAAdapter(agent: Agent, rp: RequestPromiseModule)\\: ApiAiVAAdapter} \\		Costruttore che realizza una dependency injection avente come oggetto un \file{Agent};\\
		Parametri:
		\begin{itemize}
			\item \file{agent: Agent} \\
...
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/rich-messages} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ text: String} \\
...
Per la relativa documentazione, consultare questa pagina \\ \url{https://docs.api.ai/docs/concept-contexts} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
Per chiarire quanto appena detto, consultare la relativa documentazione in questa pagina \url{https://api.ai/blog/2015/11/23/Contexts/}  (visitato in data 2017-04-22);
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
...
Per chiarire quanto appena detto, consultare la relativa documentazione \url{https://docs.api.ai/docs/webhook}  (visitato in data 2017-04-22).
;
	\item \textbf{Attributi}:
	\begin{itemize}
...
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/query#response}  (visitato in data 2017-04-22);
	...
	\begin{itemize}
		...
		Attributo contenente un valore booleano che indica se è stato usato un webhook. Per chiarire quanto detto, consultare la relativa documentazione \url{https://docs.api.ai/docs/webhook}  (visitato in data 2017-04-22);
...
Per chiarire quanto detto, consultare la relativa documentazione \url{https://docs.api.ai/docs/webhook#webhook-for-slot-filling}  (visitato in data 2017-04-22);
...
\url{https://docs.api.ai/docs/rich-messages}  (visitato in data 2017-04-22)\\
\url{https://docs.api.ai/docs/query#section-message-objects} (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ type: int} \\
...
Per la relativa documentazione, consultare la pagina \url{https://docs.api.ai/docs/concept-events}  (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ name: String} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo la cui presenza è obbligatoria, a meno che non sia presente l'attributo \file{text}. Indica l'evento di cui si richiede l'esecuzione. \\ Per una definizione di evento fare riferimento alla documentazione di api.ai alla pagina \url{https://docs.api.ai/docs/concept-events} (visitato in data 2017-04-22);
		\item[] \file{+ session\_id: String} \\
...
Essa è legata alle classi \file{ApplicationRegistryLocalClient}, \file{ApplicationLocalRegistry} e \file{Manager}. Insieme, queste classi, permettono il recupero di una applicazione dal registro. \\
L'interfaccia in oggetto, come si può vedere nel diagramma \ref{fig:sidedisc}, fornisce l'interfaccia a \file{Manager} per interagire con il registro.
;
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ register(name: String, pkg: ApplicationPackage): ErrorPromise} \\\\		Metodo che permette la registrazione di un'applicazione nell'\file{ApplicationRegistryClient}. Questo metodo deve però impedire l'inserimento di package "Parziali" di RegistryPackage, ovvero un package di tipo \file{ApplicationPackage};\\
		Parametri:
		\begin{itemize}
			\item \file{name: String} \\
		...
		\end{itemize}
		\item[] \file{+ query(name: String): ApplicationPackagePromise} \\		Metodo che permette di ottenere il package di un'applicazione dal \file{Registry}. Restituisce null in caso l'applicazione non sia disponibile;\\
		Parametri:
		\begin{itemize}
			\item \file{name: String} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo contenente l'\file{HTMLElement} dell'interfaccia dell'applicazione. Per informazioni sul tipo \file{HTMLElement}, fare riferimento alla pagina seguente:\url{https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement}  (visitato in data 2017-04-22);
		\item[] \file{- name: String} \\
...
Essa è legata all'interfaccia \file{ApplicationRegistryClient} e alle classi \\ \file{ApplicationRegistryLocalClient} e \file{Manager}. Insieme, queste classi, permettono il recupero di una applicazione dal registro. \\
La classe in oggetto, come si può vedere nel diagramma \ref{fig:sidedisc}, è il registro che viene interrogato da \file{Manager} tramite l'interfaccia \file{ApplicationRegistryClient}.
;
	\item \textbf{Attributi}:
	\begin{itemize}
...
Essa è legata all'interfaccia \file{ApplicationRegistryClient} e alle classi \file{Manager} e \file{ApplicationLocalRegistry}. Insieme, queste classi, permettono il recupero di una applicazione dal registro. \\
La classe in oggetto, come si può vedere nel diagramma \ref{fig:sidedisc}, implementa l'interfaccia utilizzata da \file{Manager} per interagire con il registro.
;
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ query(app\_name: String): ApplicationPackageObservable} \\		Metodo che permette di interrogare \file{ApplicationRegistryClient} a partire dal nome dell'applicazione, ottenendo l'\file{ApplicationPackage} relativa ad essa;\\
		Parametri:
		\begin{itemize}
			\item \file{app\_name: String} \\
			Parametro contenente il nome dell'applicazione che si vuole recuperare;
		\end{itemize}
		\item[] \file{+ register(name: String, pkg: ApplicationPackage): ErrorObservable}\\ \\		Metodo relativo alla registrazione di un'applicazione nell'\file{ApplicationRegistryClient}.
Questo metodo deve però impedire l'inserimento di package "Parziali" di \file{RegistryPackage}, ovvero un package di tipo \file{ApplicationPackage};\\
		...
		\end{itemize}
		\item[] \file{+ <<Create>> createApplicationRegistryLocalClient(registry: \\ ApplicationLocalRegistry)\\: ApplicationRegistryLocalClient} \\		Costruttore che permette di effettuare una dependency injection di \\ \file{ApplicationLocalRegistry};\\
		Parametri:
		\begin{itemize}
			\item \file{registry: ApplicationLocalRegistry} \\
...
Essa è legata all'interfaccia \file{ApplicationRegistryClient} e alle classi \\ \file{ApplicationRegistryLocalClient} e \file{ApplicationLocalRegistry}. Insieme, queste classi, permettono il recupero di una applicazione dal registro. \\
La classe in oggetto, come si può vedere nel diagramma \ref{fig:sidedisc}, sfrutta l'interfaccia sul registro per interagire con quest'ultimo.
 ...
 \begin{figure}[h]
	 	...
	 	\caption{Organizzazione delle classi che interagiscono con il registro e ne forniscono l'interfaccia.}\label{fig:sidedisc}
\end{figure};
	...
	\begin{itemize}
		...
		Attributo che contiene l'elemento del \gl{DOM} al quale verrà appesa la view dell'applicazione. Per informazoni sul tipo \file{HTMLElement} fare riferimento alla pagina \url{https://developer.mozilla.org/en/docs/Web/API/HTMLElement}  (visitato in data 2017-04-22);
		\item[] \file{- application: Application} \\
		Attributo che contiene l'applicazione attualmente in esecuzione;
		\item[] \file{- ui: HTMLElement} \\
		Attributo contenente l'\file{HTMLElement} dell'interfaccia dell'applicazione. Per informazioni sul tipo \file{HTMLElement}, fare riferimento alla pagina seguente:\url{https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement} (visitato in data 2017-04-22);
	\end{itemize}
	\item \textbf{Metodi}:
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ \_changeApplication(app: Application): void} \\		Metodo che permette di aggiornare l'interfaccia dell'applicazione con la nuova \file{Application} richiesta;\\
		Parametri:
		\begin{itemize}
			\item \file{app: Application} \\
			Parametro contenente la nuova \file{Application} da istanziare;
		\end{itemize}
	\end{itemize}
	\item \textbf{Relazioni con le altre classi}:
	\begin{itemize}
...
\item notificare gli observer collegati che è stata decisa la \file{ConversationAction};
\item gestire i comandi da eseguire da \file{ConversationApp}.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Utilizzo}: rappresenta il Dispatcher nel pattern Flux. Fornisce agli Store i meccanismi necessari ad iscriversi e si occupa di notificarli quando arriva una \file{Action}. Permette inoltre alle altre classi di generare \file{Action};
	\item \textbf{Attributi}:
	\begin{itemize}
...
\item il notificare alle classi interessate l'arrivo del testo di risposta dal \file{Back-end};
\item l'esecuzione di richieste HTTP sostituendo le funzioni di Callback fornite da \gl{Javascript} con le Promise che sono più chiare in casi di annidamento;
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Tipo}: \file{Class};
	\item \textbf{Descrizione}: classe che rappresenta un \file{Observable} che notifica gli observer interessati quando sono arrivati dei dati dall'API Gateway;
...
\end{itemize};
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ next(val: respondeBody): void} \\		Questo metodo permette di notificare il \file{PlayerObserver} e l'\file{ApplicationManagerObserver};\\
		Parametri:
		\begin{itemize}
			\item \file{val: respondeBody} \\
			Parametro contenente il valore con il quale notificare il \file{PlayerObserver} e \\ l'\file{ApplicationManagerObserver};
		\end{itemize}
		\item[] \file{+ subscribe(obs: Observer): Subscription} \\		Questo metodo permette di iscrivere il \file{PlayerObserver} e l'\file{ApplicationManagerObserver};\\
		Parametri:
		\begin{itemize}
			\item \file{obs: Observer} \\
...
\begin{itemize}
	...
	\begin{itemize}
		...
		Attributo contenente il codice di stato HTTP, come definito nella sezione 6 dello standard rfc7231 (\url{https://tools.ietf.org/html/rfc7231#section-6}  (visitato in data 2017-04-22));
		\item[] \file{+ status\_text: String} \\
...
\end{itemize};
	...
	\item \textbf{Metodi}:
	\begin{itemize}
		\item[] \file{+ sendData(audio: Blob): void} \\		Metodo che permette di inviare l'audio all'API Gateway;\\
		Parametri:
		\begin{itemize}
			\item \file{audio: Blob} \\
...
In caso il canale sia uno, allora questo attributo dovrà contenere il valore true;
		...
		\end{itemize}
		\item[] \file{- record(inputbuffer: Float32Array): void} \\		Metodo che permette di iniziare a registrare l'audio dal microfono in seguito ad un messaggio ricevuto dal \file{Recorder};\\
		Parametri:
		\begin{itemize}
			\item \file{inputbuffer: Float32Array} \\
...
Per la relativa documentazione, consultare questa pagina \url{https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesis} (visitato in data 2017-04-22);
...
Questo attributo serve per disattivare il \file{Recorder} durante la riproduzione del file audio relativo alla risposta, fornita dal sistema;
	...
	\begin{itemize}
		...
		\end{itemize}
		\item[] \file{+ isPlaying(): boolean} \\		Metodo che ritorna true se il \file{Player} sta riproducendo un audio o è in pausa;\\
...
Per la relativa documentazione, consultare la pagina \url{https://developer.mozilla.org/it/docs/Web/API/SpeechSynthesisUtterance}  (visitato in data 2017-04-22);
	\item \textbf{Attributi}:
	\begin{itemize}
		\item[] \file{+ lang: String} \\
